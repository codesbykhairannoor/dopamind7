    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        ...
        if (__props.show) {
          ...
          _push2(`<!--]--></div></div><div class="space-y-2"><label class="text-[11px] font-bold text-slate-400 dark:text-slate-600">${ssrInterpolate(_ctx.$t("vault_label_color"))}</label><div class="flex flex-wrap gap-3"><!--[-->`);
          ...
          _push2(`<!--]--></div></div></div><div class="p-8 pb-10 md:pb-8 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between border-t border-slate-100 dark:border-slate-800 transition-colors z-20 sticky bottom-0"><button class="text-[10px] font-bold text-slate-400 dark:text-slate-600 px-4 py-2 hover:text-rose-500 transition-colors">${ssrInterpolate(_ctx.$t("btn_cancel"))}</button><button${ssrIncludeBooleanAttr(__props.processing) ? " disabled" : ""} class="bg-indigo-600 text-white px-8 py-3.5 rounded-2xl font-black text-[11px] shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 hover:scale-105 active:scale-95 transition-all flex items-center gap-3">${ssrInterpolate(__props.processing ? _ctx.$t("btn_processing") : ((_b = __props.saving) == null ? void 0 : _b.id) ? _ctx.$t("vault_btn_update") : _ctx.$t("vault_btn_create"))}</button></div></div></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
