import { unref, withCtx, createBlock, createTextVNode, openBlock, createVNode, toDisplayString, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<form class="space-y-4"><div><input id="email" type="email"${ssrRenderAttr("value", unref(form).email)}${ssrRenderAttr("placeholder", _ctx.$t("auth_placeholder_email"))} class="${ssrRenderClass([unref(form).errors.email ? "border-red-400 focus:ring-red-100 focus:border-red-500" : "focus:border-indigo-500 focus:ring-indigo-500/10", "w-full px-4 py-3 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-4 transition-all outline-none"])}" required autofocus>`);
      ...
      _push(`</div></form><div class="mt-10 text-center font-medium"><a${ssrRenderAttr("href", _ctx.route("resources.help"))} class="text-[11px] text-slate-500 hover:text-indigo-600 transition-colors">${ssrInterpolate(_ctx.$t("auth_footer_help"))}</a></div></div></div><!--]-->`);
    };
  }
};
