...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "bg-white p-6 rounded-3xl border border-slate-100 shadow-sm h-fit flex flex-col gap-6" }, _attrs))}><div class="flex p-1 bg-slate-50 rounded-xl border border-slate-100"><button class="${ssrRenderClass([activeTab.value === "expense" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600", "flex-1 py-2 text-[11px] font-black uppercase tracking-widest rounded-lg transition-all"])}">${ssrInterpolate(_ctx.$t("expense"))}</button><button class="${ssrRenderClass([activeTab.value === "income" ? "bg-white text-emerald-600 shadow-sm" : "text-slate-400 hover:text-slate-600", "flex-1 py-2 text-[11px] font-black uppercase tracking-widest rounded-lg transition-all"])}">${ssrInterpolate(_ctx.$t("income"))}</button></div>`);
      if (activeTab.value === "expense") {
        ...
        if (__props.budgets.length === 0) {
          ...
          _push(`<div class="space-y-4"><div class="p-4 rounded-xl border border-indigo-100 bg-indigo-50/30 flex justify-between items-center shadow-inner"><div class="flex flex-col"><span class="text-[11px] uppercase font-bold text-indigo-400 tracking-wider">Total Budget</span><span class="text-sm font-black text-slate-700 mt-0.5">${ssrInterpolate(unref(formatMoney)(totalBudget.value))}</span></div><div class="text-right flex flex-col"><span class="text-[11px] uppercase font-bold text-slate-400 tracking-wider">Terpakai</span><span class="${ssrRenderClass([totalBudgetExpense.value > totalBudget.value ? "text-rose-500" : "text-indigo-600", "text-sm font-black mt-0.5"])}">${ssrInterpolate(unref(formatMoney)(totalBudgetExpense.value))}</span></div></div><div class="space-y-3"><!--[-->`);
