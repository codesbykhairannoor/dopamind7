...
import _sfc_main$2 from "./PlannerSidebar-Ce2pKczP.js";
  ...
  const submitTask = () => {
    ...
    if (isEditing.value) {
      ...
      router.patch(route("planner.update", form.id), payload, {
        ...
        only: ["tasks", "errors"],
        // ✅ Jangan load ulang halaman penuh
        onFinish: () => form.reset()
      });
    } else {
...
function usePlannerBatch(currentDateRef) {
  ...
  const submitBatch = () => {
    ...
    batchForm.post(route("planner.batchStore"), {
      ...
      preserveState: true,
      progress: false,
      // ✅ Matikan loading default
      only: ["tasks", "errors"],
      // ✅ Tarik data Planner (tasks) saja!
      onSuccess: () => {
        closeBatchModal();
        Swal.fire({
          toast: true,
