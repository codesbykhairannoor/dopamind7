    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        ...
        if (__props.showCreateModal) {
          ...
          _push2(`</div><button class="bg-slate-100 dark:bg-slate-800 w-8 h-8 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center transition -mt-6">✕</button></div><form class="space-y-5"><div><label class="text-[10px] font-bold text-slate-400 dark:text-slate-500 mb-2 block ml-1">${ssrInterpolate(_ctx.$t("habit_modal_question"))}</label><input${ssrRenderAttr("value", __props.form.name)} class="w-full text-base md:text-lg font-bold py-3 px-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 focus:border-indigo-500 transition" placeholder="Contoh: Lari Pagi" required></div><div><label class="text-[10px] font-bold text-slate-400 dark:text-slate-500 mb-2 block ml-1">${ssrInterpolate(_ctx.$t("habit_modal_icon"))}</label><div class="grid grid-cols-6 gap-2 bg-slate-50 dark:bg-slate-950/50 p-2 rounded-xl border border-slate-100 dark:border-slate-800 max-h-32 md:max-h-40 overflow-y-auto custom-scrollbar"><!--[-->`);
          ...
          _push2(`<!--]--></div></div><div class="flex gap-4 md:gap-6"><div class="flex-1"><label class="text-[10px] font-bold text-slate-400 dark:text-slate-500 mb-2 block ml-1">${ssrInterpolate(_ctx.$t("habit_modal_color"))}</label><div class="flex flex-wrap gap-2"><!--[-->`);
