  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(currentImageUrl)) {
        _push(`<div class="relative rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm"><img${ssrRenderAttr("src", unref(currentImageUrl))} alt="Journal Image" class="w-full h-auto max-h-[500px] object-cover"><button type="button" class="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur text-rose-500 rounded-full flex items-center justify-center shadow-lg hover:bg-rose-500 hover:text-white transition-all opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100 z-10 cursor-pointer"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div>`);
      } else {
        _push(`<button class="w-full py-8 border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center text-slate-400 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-600 transition-all cursor-pointer relative overflow-hidden group">`);
        if (unref(isExplorer)) {
          _push(`<div class="absolute inset-0 bg-slate-50/50 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center"><span class="text-xl mb-1">🔒</span><span class="text-[8px] font-black text-indigo-600 uppercase tracking-[0.2em]">${ssrInterpolate(_ctx.$t("premium_badge", "Architect Feature"))}</span></div>`);
        } else {
          _push(`<!---->`);
        }
