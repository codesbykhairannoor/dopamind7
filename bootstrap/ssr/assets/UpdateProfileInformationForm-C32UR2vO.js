...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(_attrs)}><header><h2 class="text-lg font-black text-slate-800 dark:text-white uppercase tracking-widest transition-colors duration-500">${ssrInterpolate(_ctx.$t("profile_info_title", "Informasi Pribadi"))}</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400 font-medium transition-colors duration-500">${ssrInterpolate(_ctx.$t("profile_info_desc", "Perbarui foto profil, nama, dan alamat email akun Anda."))}</p></header><div class="mt-8 space-y-6"><div class="flex items-center gap-6 p-5 bg-slate-50 dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 transition-colors duration-500"><div class="relative group cursor-pointer shrink-0"><img${ssrRenderAttr("src", photoPreview.value || user.value.avatar_url)} alt="Profile" class="w-24 h-24 rounded-full object-cover shadow-md dark:shadow-none border-4 border-white dark:border-slate-800 transition-all duration-300 group-hover:brightness-75 group-hover:scale-105">`);
      ...
      _push(ssrRenderComponent(_sfc_main$2, {
        ...
        class: "!text-[10px] !font-black !uppercase !tracking-widest !text-slate-400 dark:!text-slate-500 mb-2 transition-colors duration-500"
      }, null, _parent));
      ...
      _push(ssrRenderComponent(PrimaryButton, {
        ...
        class: "!rounded-xl !py-3.5 !px-8 !bg-indigo-600 hover:!bg-indigo-700 !shadow-lg !shadow-indigo-200 dark:!shadow-none !text-[11px] !font-black !uppercase !tracking-widest transition-all active:scale-95 transition-colors duration-500"
      }, {
      ...
      if (unref(form).recentlySuccessful) {
        _push(`<p class="text-[11px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-100 dark:border-emerald-500/20 transition-colors duration-500"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg> ${ssrInterpolate(_ctx.$t("saved", "Tersimpan"))}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></form></div>`);
      ssrRenderTeleport(_push, (_push2) => {
        if (showCropper.value) {
          ...
          })}"></div><p class="mt-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest transition-colors duration-500">${ssrInterpolate(_ctx.$t("pan_instruction", "Gunakan Mouse/Sentuhan Untuk Menggeser"))}</p><div class="w-full mt-6 flex items-center gap-3"><span class="text-sm text-slate-400 dark:text-slate-500 transition-colors duration-500">🔍</span><input type="range"${ssrRenderAttr("value", zoom.value)} min="0.5" max="2.5" step="0.05" class="w-full accent-indigo-600 dark:accent-indigo-400"></div><div class="flex gap-3 mt-8 w-full"><button type="button" class="flex-1 py-3.5 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-2xl font-black text-xs uppercase tracking-widest border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 active:scale-95 transition-all transition-colors duration-500">${ssrInterpolate(_ctx.$t("cancel", "Batal"))}</button><button type="button" class="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 active:scale-95 transition-all">${ssrInterpolate(_ctx.$t("save_photo", "Simpan Foto"))}</button></div></div></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
