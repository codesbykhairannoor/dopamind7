...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative z-[60] bg-white/80 backdrop-blur-xl border-b border-slate-200 transition-all md:sticky md:top-0" }, _attrs))}><div class="px-4 sm:px-6 lg:px-8 py-4"><div class="w-full flex flex-col md:flex-row justify-between items-center gap-4"><div class="flex items-center gap-4 w-full md:w-auto"><div class="w-12 h-12 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-200 shrink-0">`);
      _push(ssrRenderComponent(_sfc_main$1, {
        name: "planner",
        size: "28"
      }, null, _parent));
      _push(`</div><div><h2 class="text-xl md:text-2xl font-black text-slate-800 tracking-tight leading-tight">${ssrInterpolate(_ctx.$t("header_title", "Daily Planner"))}</h2><div class="flex items-center mt-1 relative"><div class="relative"><button class="${ssrRenderClass([{ "bg-indigo-50 text-indigo-700": showDatePicker.value }, "text-slate-600 hover:text-indigo-600 font-bold text-sm capitalize transition-colors flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-slate-50"])}">${ssrInterpolate(__props.formattedDate)} `);
      _push(ssrRenderComponent(_sfc_main$1, {
        name: "chevron-down",
        size: "10",
        "stroke-width": "3",
        class: ["text-slate-400 transition-transform duration-300", { "rotate-180": showDatePicker.value }]
      }, null, _parent));
      _push(`</button>`);
