  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(categoryDistribution.value, (stat, idx) => {
        _push(`<div style="${ssrRenderStyle({ width: stat.count / ((__props.localTaskBox || []).length || 1) * 100 + "%" })}" class="${ssrRenderClass([stat.color, "transition-all duration-1000 h-full first:rounded-l-full last:rounded-r-full"])}"></div>`);
      });
