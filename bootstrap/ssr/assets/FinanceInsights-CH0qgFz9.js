...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mt-6 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700" }, _attrs))} data-v-55375a3c><div class="bg-indigo-900 dark:bg-slate-950 rounded-[2rem] p-1 shadow-2xl dark:shadow-none shadow-indigo-200/50 dark:shadow-indigo-900/20 relative overflow-hidden group transition-all duration-500" data-v-55375a3c><div class="absolute inset-0 bg-gradient-to-br from-indigo-600 to-indigo-900 dark:from-indigo-900 dark:to-slate-950 transition-all duration-500" data-v-55375a3c></div><div class="absolute -right-10 -top-10 w-32 h-32 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-all duration-1000" data-v-55375a3c></div><div class="relative z-10 p-5" data-v-55375a3c><div class="flex justify-between items-start mb-6" data-v-55375a3c><div data-v-55375a3c><div class="flex items-center gap-1.5 mb-1.5" data-v-55375a3c><span class="text-lg" data-v-55375a3c>🧪</span><p class="text-indigo-200 text-[9px] font-black uppercase tracking-[0.2em]" data-v-55375a3c>${ssrInterpolate(_ctx.$t("investment_lab"))}</p></div><h4 class="text-2xl font-black text-white tracking-tighter" data-v-55375a3c>${ssrInterpolate(unref(formatMoney)(portfolioSummary.value.currentValue))}</h4><div class="inline-flex items-center gap-1.5 mt-1 bg-black/20 px-2.5 py-1 rounded-md border border-white/5 backdrop-blur-sm" data-v-55375a3c><span class="${ssrRenderClass([portfolioSummary.value.totalReturn >= 0 ? "bg-emerald-400" : "bg-rose-400", "w-1.5 h-1.5 rounded-full"])}" data-v-55375a3c></span><p class="${ssrRenderClass([portfolioSummary.value.totalReturn >= 0 ? "text-emerald-300" : "text-rose-300", "text-[9px] font-bold"])}" data-v-55375a3c>${ssrInterpolate(portfolioSummary.value.totalReturn >= 0 ? "+" : "")}${ssrInterpolate(unref(formatMoney)(portfolioSummary.value.totalReturn))} ${ssrInterpolate(_ctx.$t("total_pl"))}</p></div></div><button class="w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20 backdrop-blur-md flex items-center justify-center text-xl text-white border border-white/20 transition-all active:scale-95 shadow-md" data-v-55375a3c> ＋ </button></div>`);
      if (assets.value.length > 0) {
        _push(`<div class="space-y-2 max-h-[350px] overflow-y-auto custom-scrollbar pr-1" data-v-55375a3c><!--[-->`);
        ssrRenderList(assets.value, (asset) => {
          _push(`<div class="bg-indigo-950/50 dark:bg-black/20 border border-indigo-500/30 dark:border-white/5 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group/card hover:bg-indigo-800/80 dark:hover:bg-white/5 transition-all transition-colors duration-500" data-v-55375a3c><div class="flex flex-col gap-2 flex-1 min-w-0" data-v-55375a3c><div class="flex items-center gap-2" data-v-55375a3c><h5 class="text-sm font-black text-white truncate" data-v-55375a3c>${ssrInterpolate(asset.name)}</h5><span class="text-[8px] font-bold px-1.5 py-0.5 rounded bg-indigo-500/30 text-indigo-200 border border-indigo-500/30" data-v-55375a3c>${ssrInterpolate(unref(formatMoney)(asset.capital))}</span></div><div class="flex items-center w-fit bg-black/30 rounded-lg border border-white/10 overflow-hidden focus-within:border-yellow-400/50 transition-all" data-v-55375a3c><div class="bg-white/5 px-2 py-1 border-r border-white/10" data-v-55375a3c><svg class="w-3 h-3 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-55375a3c><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" data-v-55375a3c></path></svg></div><input type="number"${ssrRenderAttr("value", asset.percent)} class="w-16 bg-transparent border-none text-sm font-black text-center text-yellow-400 focus:outline-none focus:ring-0 py-1 px-1 placeholder:text-yellow-700/50" placeholder="0" data-v-55375a3c><div class="bg-white/5 px-2 py-1 border-l border-white/10" data-v-55375a3c><span class="text-[8px] font-bold text-indigo-200 uppercase" data-v-55375a3c>${ssrInterpolate(_ctx.$t("growth"))}</span></div></div></div><div class="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center w-full sm:w-auto pt-2 sm:pt-0 mt-1 sm:mt-0 border-t sm:border-none border-indigo-500/20 gap-2" data-v-55375a3c><div class="text-left sm:text-right" data-v-55375a3c><p class="text-[8px] text-indigo-300 mb-0.5 uppercase tracking-widest font-bold" data-v-55375a3c>${ssrInterpolate(_ctx.$t("est_total"))}</p><p class="${ssrRenderClass([asset.percent >= 0 ? "text-emerald-400" : "text-rose-400", "text-sm font-black tracking-tight"])}" data-v-55375a3c>${ssrInterpolate(unref(formatMoney)(asset.capital * (1 + asset.percent / 100)))}</p></div><button class="text-[9px] font-black tracking-widest text-white bg-rose-500/90 hover:bg-rose-500 px-4 py-1.5 rounded-lg transition-all shadow-sm active:scale-95" data-v-55375a3c>${ssrInterpolate(_ctx.$t("quit"))}</button></div></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<div class="py-8 flex flex-col items-center justify-center border border-dashed border-indigo-400/30 rounded-xl bg-indigo-950/30 mt-2" data-v-55375a3c><span class="text-2xl mb-2 opacity-50 grayscale" data-v-55375a3c>🕸️</span><p class="text-[10px] font-bold text-indigo-300 uppercase tracking-[0.2em]" data-v-55375a3c>${ssrInterpolate(_ctx.$t("lab_empty"))}</p></div>`);
      }
      _push(`</div></div></div>`);
    };
  }
...
const FinanceInsights = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-55375a3c"]]);
