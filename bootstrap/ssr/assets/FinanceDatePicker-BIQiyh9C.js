...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (__props.show) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "w-[300px] sm:w-[320px] max-w-[95vw]" }, _attrs))}><div class="fixed inset-0 z-40 hidden sm:block"></div><div class="relative z-50 bg-white dark:bg-slate-900 rounded-3xl shadow-xl dark:shadow-none border border-slate-100 dark:border-slate-800 p-5 animate-in fade-in zoom-in-95 duration-200 transition-colors duration-500"><div class="flex items-center justify-between mb-4"><button class="p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition duration-300">◀</button><h4 class="font-bold text-slate-700 dark:text-white capitalize transition-colors duration-500">${ssrInterpolate(formattedHeader.value)}</h4><button class="p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition duration-300">▶</button></div><div class="grid grid-cols-7 mb-2 text-center"><!--[-->`);
        ssrRenderList(weekDays.value, (dayName) => {
          _push(`<span class="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase transition-colors duration-500">${ssrInterpolate(dayName)}</span>`);
        });
        ...
        ssrRenderList(daysInMonth.value, (day) => {
          _push(`<button class="${ssrRenderClass([[
            isSelected(day) ? "bg-indigo-600 text-white shadow-lg dark:shadow-none shadow-indigo-200 dark:shadow-indigo-900/40" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-indigo-400",
            isToday(day) && !isSelected(day) ? "ring-1 ring-indigo-200 dark:ring-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-500/10" : ""
          ], "h-9 w-9 rounded-xl flex flex-col items-center justify-center transition-all relative group transition-all duration-300"])}"${ssrRenderAttr("title", getDayStats(day) ? `${unref(formatMoney)(getDayStats(day).net)} (${getDayStats(day).count} Trx)` : "Tidak ada transaksi")}><span class="text-sm font-bold leading-none">${ssrInterpolate(day)}</span>`);
          if (getDayStats(day)) {
