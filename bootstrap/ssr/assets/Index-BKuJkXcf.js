...
import { Wallet, Plus, TrendingUp } from "lucide-vue-next";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(isExplorer)) {
        ...
        _push(`</div><div><h3 class="text-base lg:text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">The Vault</h3><p class="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mt-0.5">Your wealth manifestation</p></div></div><button class="flex items-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-3 py-2 lg:px-4 rounded-xl text-[9px] lg:text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all active:scale-95 shadow-lg dark:shadow-none">`);
        _push(ssrRenderComponent(unref(Plus), { size: 14 }, null, _parent));
        _push(`<span class="hidden sm:inline">Create Goal</span><span class="sm:hidden">Goal</span></button></div><div class="grid grid-cols-1 md:grid-cols-2 gap-6">`);
        ...
        _push(`</div><div><h4 class="text-xs font-black text-white uppercase tracking-widest">Neural Forecast</h4><p class="text-[9px] font-bold text-indigo-300 uppercase tracking-tighter">AI Wealth Prediction</p></div></div><div class="px-2 py-1 rounded-md bg-indigo-500 text-white text-[8px] font-black uppercase tracking-widest shadow-lg">Elite</div></div><div class="flex-1 flex items-end gap-1.5 h-24 mb-6 blur-[2px] group-hover:blur-0 transition-all duration-700 opacity-30"><!--[-->`);
        ...
        _push(`<!--]--></div><div class="mt-auto flex items-center justify-between"><span class="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Unlock Predictions</span>`);
