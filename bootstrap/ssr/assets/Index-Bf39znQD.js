import { ref, watch, computed, onMounted, unref, isRef, useSSRContext } from "vue";
...
import _sfc_main$1 from "./PlannerHeader-D3cv-AWu.js";
import _sfc_main$3 from "./PlannerSidebar-BUTNmmcp.js";
import PlannerTimeline from "./PlannerTimeline-DHlmGrse.js";
import "./PlannerMobileTimeline-Dm86BcmR.js";
import _sfc_main$4 from "./PlannerModal-CREn3Cta.js";
import PlannerBatchModal from "./PlannerBatchModal-ZD3F4rym.js";
import { _ as _sfc_main$2 } from "./NeuralBridge-BT8K7-5N.js";
import "./OneForMindIcon-XdjRmiFl.js";
import "./ThemeToggle-ByQSx4Ee.js";
import "./useAppearance-rDoGVD4_.js";
import "./_plugin-vue_export-helper-1tPrXgE0.js";
import "./PlannerDatePicker-C0E2B7JI.js";
...
function usePlannerTasks(props) {
  ...
  const scheduledTasks = computed(
    () => localTasks.value.filter((t2) => t2.start_time && t2.date === currentDateRef.value).sort((a, b) => a.start_time.localeCompare(b.start_time))
  );
  const inboxTasks = computed(
    () => localTasks.value.filter((t2) => !t2.start_time && t2.date === currentDateRef.value)
    // 🔥 FIX: Filter by date
  );
...
function usePlanner(props) {
  ...
  const localWater = ref(((_c = props.dailyLog) == null ? void 0 : _c.water) || 0);
  const localTaskBox = ref(((_d = props.dailyLog) == null ? void 0 : _d.task_box) || []);
  const activeModalType = ref("full");
  const saveLogSilent = debounce(async (data) => {
    try {
      await axios.post(route("planner.updateLog"), {
        ...data,
        date: props.currentDate
        // Pastikan nge-save ke tanggal yang benar!
      });
    } catch (e) {
  ...
  watch(() => props.dailyLog, (newLog) => {
    localNotes.value = (newLog == null ? void 0 : newLog.notes) || "";
    localMeals.value = (newLog == null ? void 0 : newLog.meals) || { breakfast: "", lunch: "", dinner: "" };
    localWater.value = (newLog == null ? void 0 : newLog.water) || 0;
    localTaskBox.value = (newLog == null ? void 0 : newLog.task_box) || [];
  }, { deep: true });
...
function usePlannerBatch(currentDateRef, localTasksRef = null) {
  ...
  const submitBatch = () => {
    ...
    batchForm.post(route("planner.batchStore"), {
      ...
      preserveState: true,
      progress: false,
      // Matikan loading bar di atas
      only: ["tasks", "errors"],
      // Tarik balasan daftar tasks yang asli dari server
      onSuccess: () => {
        batchForm.reset();
        batchForm.clearErrors();
        globalConflictError.value = null;
...
function usePlannerCalendar(initialDate) {
  const page = usePage();
  ...
  const changeDate = (newDate) => {
    ...
    router.get(route("planner.index"), { date: newDate }, {
      ...
      preserveScroll: true,
      replace: true,
      // Biar ga menuhin history back browser
      only: ["tasks", "dailyLog", "currentDate"]
      // Cuma ambil ini aja
    });
  };
  const changeDay = (direction) => {
    const current = dayjs(currentDate.value);
    const newDate = direction === "next" ? current.add(1, "day").format("YYYY-MM-DD") : current.subtract(1, "day").format("YYYY-MM-DD");
    changeDate(newDate);
  };
  return {
    currentDate,
...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: AuthenticatedLayout }, {
  ...
  setup(__props) {
    ...
    const {
      localTasks,
      // 🔥 FIX 1: Keluarkan localTasks dari sini
      scheduledTasks,
      timeSlots,
      scheduledStats,
      form,
      ...
      isEditing,
      conflictError,
      // pastikan conflictError juga diekstrak
      openModal,
      submitTask,
      deleteTask,
      resetBoard,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$1, {
        currentDate: unref(currentDate),
        formattedDate: unref(formattedDate),
        tasks: __props.tasks,
        onChangeDate: unref(changeDate),
        onChangeDay: unref(changeDay),
        onAddClick: () => unref(openModal)(null, null, "full"),
        resetBoard: handleFullReset,
        stats: unref(scheduledStats),
        isExplorer: unref(gating).isExplorer.value
      }, null, _parent));
      _push(`<div class="w-full min-h-screen bg-slate-50/50 dark:bg-slate-950 px-4 sm:px-6 lg:px-8 py-8 transition-colors duration-500">`);
      if (!isMobile.value) {
        _push(`<div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start"><div class="lg:col-span-3 order-1 lg:order-2 w-full">`);
        ...
        _push(ssrRenderComponent(_sfc_main$2, { module: "Planner" }, null, _parent));
        _push(ssrRenderComponent(_sfc_main$3, {
          ...
          localWater: unref(localWater),
          "onUpdate:localWater": ($event) => isRef(localWater) ? localWater.value = $event : null,
          localTaskBox: unref(localTaskBox),
          "onUpdate:localTaskBox": ($event) => isRef(localTaskBox) ? localTaskBox.value = $event : null
        }, null, _parent));
        _push(`</div></div>`);
      } else {
        _push(`<div class="space-y-12 pb-40 px-2">`);
        _push(ssrRenderComponent(_sfc_main$2, {
          id: "mobile-planner-neural",
          module: "Planner"
        }, null, _parent));
        _push(`<div id="mobile-wide-timeline" class="w-full overflow-hidden"><div class="flex items-center justify-between px-2 mb-4"><h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Flow Timeline</h3></div>`);
        _push(ssrRenderComponent(PlannerTimeline, {
          timeSlots: unref(timeSlots),
          scheduledTasks: unref(scheduledTasks),
          onDrop: unref(onDrop),
        ...
        _push(`</div><div id="mobile-daily-overview" class="space-y-6 pt-8 border-t border-slate-100 dark:border-slate-800"><div class="px-2"><h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Daily Overview</h3></div>`);
        _push(ssrRenderComponent(_sfc_main$3, {
          stats: unref(scheduledStats),
          localNotes: unref(localNotes),
          "onUpdate:localNotes": ($event) => isRef(localNotes) ? localNotes.value = $event : null,
