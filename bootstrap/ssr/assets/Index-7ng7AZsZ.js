  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(isExplorer)) {
        ...
        _push(`<span class="hidden sm:inline">Create Goal</span><span class="sm:hidden">Goal</span></button></div>`);
        if (localSavings.value.length === 0) {
          _push(`<div class="group bg-white dark:bg-slate-900 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800 p-10 lg:p-12 text-center transition-colors"><div class="mb-4 text-3xl lg:text-4xl transform group-hover:scale-110 transition-transform duration-500">🏦</div><h4 class="text-slate-400 font-bold text-[10px] lg:text-sm mb-4">You have no active saving goals yet.</h4><button class="text-[9px] lg:text-[10px] font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900 px-5 lg:px-6 py-2 rounded-xl lg:rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-500/5 transition-all"> Start Saving Now </button></div>`);
        } else {
