import { ref, watch, computed, onMounted, onUnmounted, resolveComponent, mergeProps, unref, useSSRContext } from "vue";
...
import _sfc_main$2 from "./HabitModals-CgvoFYU0.js";
import NeuralHabitInsight from "./NeuralHabitInsight-WzMbJYeF.js";
import { _ as _sfc_main$1 } from "./NeuralBridge-DkSDbyOa.js";
import { _ as _sfc_main$3 } from "./PremiumPreviewModal-BdH6vUzw.js";
...
import "vue-chartjs";
import "chart.js";
import "./HabitBatchModal-awlHialw.js";
...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(isExplorer)) {
        _push(`<div class="mx-4 md:mx-8 p-6 rounded-[2.5rem] bg-slate-900 border border-indigo-500/20 relative overflow-hidden group/habit-mastery cursor-pointer hover:scale-[1.01] transition-all duration-500 shadow-2xl"><div class="absolute inset-0 bg-gradient-to-br from-indigo-600/10 via-transparent to-transparent"></div><div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6"><div class="flex items-center gap-4"><div class="w-12 h-12 rounded-2xl bg-indigo-500/20 flex items-center justify-center text-indigo-400 border border-indigo-500/30">`);
        _push(ssrRenderComponent(unref(Activity), { size: 24 }, null, _parent));
        _push(`</div><div><h4 class="text-sm font-black text-white tracking-tight leading-none mb-1">Neural consistency lab</h4><p class="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Analyze your identity shifts with AI</p></div></div><div class="flex gap-1.5 blur-[1.5px] opacity-30 group-hover:opacity-60 transition-all"><!--[-->`);
        ssrRenderList(12, (i) => {
          _push(`<div class="${ssrRenderClass([i % 3 === 0 ? "bg-indigo-500" : "bg-slate-700", "w-3 h-3 rounded-[3px]"])}"></div>`);
        });
        _push(`<!--]--></div><div class="flex items-center gap-2 text-indigo-400 font-bold text-[10px] tracking-widest uppercase"> Unlock Analytics `);
        _push(ssrRenderComponent(_component_ArrowRight, { size: 14 }, null, _parent));
        _push(`</div></div></div>`);
      } else {
        _push(`<!---->`);
      }
