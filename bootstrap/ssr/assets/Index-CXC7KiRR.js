import { ref, watch, computed, onMounted, mergeProps, unref, useSSRContext } from "vue";
...
import GoalStats from "./GoalStats-DI7Isjjk.js";
import GoalCard from "./GoalCard-JUXdo-x1.js";
import GoalModal from "./GoalModal-BTCRpen3.js";
import _sfc_main$1 from "./GoalHeader-DkaBFTD-.js";
import { _ as _sfc_main$2 } from "./NeuralBridge-IX-p3eB4.js";
import dayjs from "dayjs";
import "dayjs/locale/id.js";
import "dayjs/locale/en.js";
import { Target, ChevronRight, CheckCircle2, Zap } from "lucide-vue-next";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!isMobile.value) {
        ...
        ssrRenderList(unref(localGoals), (goal) => {
          var _a;
          _push(`<div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden"><div class="p-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between gap-4"><div class="flex items-center gap-4 min-w-0"><div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0" style="${ssrRenderStyle({ backgroundColor: (goal.color || "#6366f1") + "20" })}">`);
          _push(ssrRenderComponent(unref(Target), {
            size: 24,
            style: { color: goal.color || "#6366f1" }
          }, null, _parent));
          _push(`</div><div class="truncate"><h3 class="text-base font-black text-slate-800 dark:text-white truncate">${ssrInterpolate(goal.title)}</h3><div class="flex items-center gap-2 mt-0.5"><div class="h-1 w-20 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"><div class="h-full rounded-full transition-all duration-1000" style="${ssrRenderStyle({ width: goal.progress + "%", backgroundColor: goal.color || "#6366f1" })}"></div></div><span class="text-[9px] font-black text-slate-400 tabular-nums">${ssrInterpolate(goal.progress || 0)}%</span></div></div></div><button class="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 active:scale-90 transition-all">`);
          _push(ssrRenderComponent(unref(ChevronRight), { size: 20 }, null, _parent));
          _push(`</button></div><div class="px-6 py-4 space-y-4 relative"><div class="absolute left-[39px] top-8 bottom-8 w-[2px] bg-slate-100 dark:bg-slate-800 pointer-events-none"></div><!--[-->`);
          ssrRenderList(goal.milestones, (m, idx) => {
            _push(`<div class="${ssrRenderClass([m.is_completed ? "opacity-50" : "", "relative flex items-start gap-4 transition-all"])}"><button class="${ssrRenderClass([m.is_completed ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-100 dark:border-slate-700", "w-8 h-8 rounded-full border-2 z-10 shrink-0 flex items-center justify-center transition-all bg-white dark:bg-slate-900 shadow-sm"])}">`);
            if (m.is_completed) {
              ...
              _push(`<div class="w-2 h-2 rounded-full" style="${ssrRenderStyle({ backgroundColor: goal.color || "#6366f1" })}"></div>`);
            }
            _push(`</button><div class="pt-1 flex-1 min-w-0"><p class="${ssrRenderClass([m.is_completed ? "line-through" : "", "text-xs font-black text-slate-700 dark:text-slate-200 leading-tight"])}">${ssrInterpolate(m.title)}</p>`);
            if (m.notes) {
              _push(`<p class="text-[10px] font-medium text-slate-400 mt-1 truncate">${ssrInterpolate(m.notes)}</p>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div></div>`);
          ...
          if (!((_a = goal.milestones) == null ? void 0 : _a.length)) {
            _push(`<div class="py-6 text-center italic text-[10px] font-bold text-slate-300"> No milestones defined yet. </div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
          if (goal.reward && !goal.is_completed) {
            _push(`<div class="bg-amber-50/50 dark:bg-amber-500/5 px-6 py-4 border-t border-amber-100/50 dark:border-amber-900/20 flex items-center gap-3">`);
            _push(ssrRenderComponent(unref(Zap), {
              size: 14,
              class: "text-amber-500"
            }, null, _parent));
            _push(`<p class="text-[10px] font-black text-amber-600/80 truncate">Reward: ${ssrInterpolate(goal.reward)}</p></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
