...
import JobTable from "./JobTable-BFjxpFVQ.js";
import _sfc_main$2 from "./JobStats-XE_mcy9P.js";
import _sfc_main$3 from "./JobFilterBar-CVztxkW1.js";
...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: AuthenticatedLayout }, {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="min-h-screen bg-[#f8fafc] pb-24"><div class="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 shadow-sm"><div class="w-full px-4 sm:px-6 lg:px-8 py-3.5 sm:py-5"><div class="flex items-center justify-between gap-4"><div class="flex items-center gap-2.5 sm:gap-3 min-w-0"><div class="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center text-white bg-indigo-600 rounded-xl shadow-lg shadow-indigo-100 shrink-0">`);
      _push(ssrRenderComponent(_sfc_main$1, {
        name: "job",
        size: "24",
        "stroke-width": 3
      }, null, _parent));
      _push(`</div><div class="min-w-0"><h1 class="text-lg sm:text-2xl font-black leading-tight tracking-tight text-slate-800 flex items-center gap-1.5 sm:gap-2"><span class="truncate">${ssrInterpolate(_ctx.$t("job_page_title", "Job Tracker"))}</span>`);
      if ((_a = __props.pagination) == null ? void 0 : _a.total) {
        _push(`<span class="text-[10px] sm:text-sm font-black text-indigo-600 bg-indigo-50 px-2 sm:px-2.5 py-0.5 rounded-full shrink-0">${ssrInterpolate(__props.pagination.total)}</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</h1><p class="mt-0.5 text-[10px] sm:text-sm font-black uppercase tracking-widest text-slate-400 truncate">${ssrInterpolate(todayDate.value)}</p></div></div><button class="bg-indigo-600 text-white font-black py-2.5 px-4 sm:px-6 rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-2 shrink-0">`);
      _push(ssrRenderComponent(_sfc_main$1, {
        name: "plus",
        size: "18",
        "stroke-width": "4"
