import { ssrInterpolate, ssrRenderAttr, ssrRenderList, ssrRenderClass, ssrRenderStyle, ssrIncludeBooleanAttr } from "vue/server-renderer";
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.showCreateModal) {
        ...
        _push(`<!--]--></div></div><div class="flex gap-4 md:gap-6"><div class="flex-1"><label class="text-xs font-bold text-slate-400 uppercase mb-2 block">${ssrInterpolate(_ctx.$t("habit_modal_color"))}</label><div class="flex flex-wrap gap-2"><!--[-->`);
        ssrRenderList(__props.colorPalette, (c) => {
          _push(`<button type="button" class="w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center transition hover:scale-110" style="${ssrRenderStyle({ backgroundColor: c })}">`);
          if (__props.form.color === c) {
            _push(`<span class="text-white font-bold text-xs">✓</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</button>`);
        });
        _push(`<!--]--></div></div><div class="flex-1"><label class="text-xs font-bold text-slate-400 uppercase mb-2 block">${ssrInterpolate(_ctx.$t("habit_modal_target"))}: ${ssrInterpolate(__props.form.monthly_target)}</label><input${ssrRenderAttr("value", __props.form.monthly_target)} type="range" min="1" max="31" class="w-full accent-indigo-600 h-2 bg-slate-200 rounded-lg cursor-pointer"></div></div><div class="flex gap-3 pt-2">`);
        if (__props.isEditing) {
          _push(`<button type="button" class="px-4 py-3 rounded-xl font-bold text-rose-500 bg-rose-50 hover:bg-rose-100 transition flex items-center justify-center border border-rose-100" title="Hapus Habit"> 🗑️ </button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<button type="submit" class="flex-1 py-3 md:py-4 rounded-xl font-bold text-white bg-indigo-950 hover:bg-indigo-900 shadow-xl transition transform hover:-translate-y-1"${ssrIncludeBooleanAttr(__props.form.processing) ? " disabled" : ""}>${ssrInterpolate(__props.isEditing ? "Update Habit" : _ctx.$t("habit_modal_btn_save"))}</button></div></form></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<!--]-->`);
