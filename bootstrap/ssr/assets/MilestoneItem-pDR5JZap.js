...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const editTitle = ref(((_a = props.milestone) == null ? void 0 : _a.title) || "");
    const editTargetDate = ref(((_b = props.milestone) == null ? void 0 : _b.target_date) || null);
    ref(false);
    computed(() => {
    ...
    watch(() => {
      var _a2;
      return (_a2 = props.milestone) == null ? void 0 : _a2.title;
    }, (newTitle) => {
    ...
    watch(() => {
      var _a2;
      return (_a2 = props.milestone) == null ? void 0 : _a2.target_date;
    }, (newDate) => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))} data-v-8562b91b><button class="${ssrRenderClass([__props.milestone.is_completed || __props.milestone.completed ? "bg-indigo-600 border-indigo-600 text-white shadow-sm shadow-indigo-200 dark:shadow-indigo-900/40" : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500", "flex-shrink-0 w-[18px] h-[18px] rounded-lg border-2 flex items-center justify-center transition-all duration-300 focus:outline-none"])}" data-v-8562b91b>`);
      ...
      _push(`</button><div class="flex-grow flex flex-col min-w-0" data-v-8562b91b><input${ssrRenderAttr("value", editTitle.value)}${ssrRenderAttr("placeholder", t("milestone_placeholder", "Identify next step..."))} class="${ssrRenderClass([__props.milestone.is_completed || __props.milestone.completed ? "text-slate-400 dark:text-slate-500 line-through font-medium" : "text-slate-700 dark:text-slate-200 font-black", "w-full bg-transparent border-none focus:ring-0 p-0 text-sm placeholder:text-slate-300 dark:placeholder:text-slate-600 transition-all font-bold"])}" data-v-8562b91b></div><button class="opacity-0 group-hover/ms:opacity-100 p-1.5 text-slate-300 dark:text-slate-600 hover:text-rose-500 dark:hover:text-rose-400 transition-all rounded-xl hover:bg-rose-50 dark:hover:bg-rose-500/10 focus:outline-none shrink-0" data-v-8562b91b>`);
      _push(ssrRenderComponent(unref(Trash2), { size: 14 }, null, _parent));
      _push(`</button>`);
      if (__props.milestone.is_saving) {
        _push(`<div class="absolute right-3 bottom-1" data-v-8562b91b><div class="w-2 h-2 border-2 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin" data-v-8562b91b></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const MilestoneItem = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-8562b91b"]]);
export {
