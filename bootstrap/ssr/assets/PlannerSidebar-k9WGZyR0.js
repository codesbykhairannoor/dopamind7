...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</button></div></div></div><div class="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800 transition-colors duration-500"><div class="flex min-w-0 justify-between items-center gap-3 mb-5"><div class="flex min-w-0 flex-1 items-center gap-2 sm:gap-3"><div class="w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-lg sm:text-xl">📥</div><div class="min-w-0"><h3 class="font-black text-slate-800 dark:text-white text-sm tracking-tight transition-colors duration-500 truncate">${ssrInterpolate(_ctx.$t("sidebar_inbox_title"))}</h3><p class="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-tighter italic transition-colors duration-500 truncate">${ssrInterpolate(_ctx.$t("sidebar_persistent"))}</p></div></div><button class="shrink-0 p-2.5 rounded-xl bg-slate-900 text-white hover:bg-indigo-600 transition-all active:scale-90 shadow-md"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M12 4v16m8-8H4"></path></svg></button></div>`);
