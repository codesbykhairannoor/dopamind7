...
import { _ as _sfc_main$6 } from "./NeuralBridge-DkSDbyOa.js";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!isMobile.value) {
        ...
        _push(`<div class="space-y-10 pb-40"><div class="px-2">`);
        _push(ssrRenderComponent(_sfc_main$1, {
          stats: localStats.value,
          onUpdateTarget: handleUpdateTarget
        }, null, _parent));
        _push(`</div><div class="space-y-6 px-2"><div class="flex items-center justify-between"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500">`);
        _push(ssrRenderComponent(unref(Wallet), { size: 18 }, null, _parent));
        _push(`</div><h3 class="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-white">The Vault</h3></div><button class="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all"> + Goal </button></div>`);
        if (unref(isExplorer)) {
          _push(`<div class="bg-white/40 dark:bg-slate-900/40 backdrop-blur-md rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 text-center cursor-pointer active:scale-95 transition-all"><span class="text-3xl mb-2 block">🔒</span><h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Upgrade to Architect to use Vault</h4></div>`);
        } else if (localSavings.value.length === 0) {
          _push(`<div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800 p-10 text-center"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">No active saving goals</p><button class="text-[9px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900 px-5 py-2 rounded-xl"> Start Saving </button></div>`);
        } else {
          _push(`<div class="flex overflow-x-auto no-scrollbar gap-4 pb-4 -mx-2 px-2"><!--[-->`);
          ssrRenderList(localSavings.value, (saving) => {
            _push(`<div class="shrink-0 w-[240px]">`);
            _push(ssrRenderComponent(_sfc_main$4, {
              saving,
              onDeposit: (s) => openVaultAction(s, "deposit"),
              onWithdraw: (s) => openVaultAction(s, "withdraw"),
        ...
        _push(`</div><div class="space-y-6 px-2"><div class="flex items-center justify-between"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-500">`);
        _push(ssrRenderComponent(_sfc_main$2, {
          name: "calendar-history",
          size: "18",
          "stroke-width": "2.5"
        }, null, _parent));
        _push(`</div><h3 class="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-white">Recent Activity</h3></div><button class="text-[10px] font-black text-indigo-600 py-1 px-3 bg-indigo-50 dark:bg-indigo-500/10 rounded-lg">See All</button></div>`);
        if (localTransactions.value.length === 0) {
          _push(`<div class="py-16 text-center bg-white dark:bg-slate-900 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800"><p class="text-xs font-bold text-slate-400 uppercase tracking-widest">No activity found</p></div>`);
        } else {
          _push(`<div class="space-y-3"><!--[-->`);
          ssrRenderList(localTransactions.value.slice(0, 8), (trx) => {
            _push(`<div class="group bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-50 dark:border-slate-800/50 flex items-center justify-between active:scale-[0.97] transition-all duration-300"><div class="flex items-center gap-4"><div class="w-10 h-10 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-xl">${ssrInterpolate(trx.category_icon || "💸")}</div><div class="min-w-0"><h4 class="text-xs font-black text-slate-800 dark:text-white truncate">${ssrInterpolate(trx.title)}</h4><p class="text-[9px] font-bold text-slate-400 mt-0.5">${ssrInterpolate(unref(dayjs)(trx.date).format("DD MMM"))} • ${ssrInterpolate(trx.category_name)}</p></div></div><div class="text-right"><p class="${ssrRenderClass([trx.type === "income" ? "text-indigo-600 dark:text-indigo-400" : "text-slate-900 dark:text-white", "text-sm font-black font-mono tracking-tighter"])}">${ssrInterpolate(trx.type === "income" ? "+" : "-")}${ssrInterpolate(unref(formatMoney)(trx.amount))}</p></div></div>`);
          });
          _push(`<!--]--></div>`);
        }
        _push(`</div><div class="px-2">`);
        _push(ssrRenderComponent(FinanceInsights, {
          "expense-stats": localStats.value.expense_by_category,
          "income-stats": localStats.value.income_by_category,
          budgets: localBudgets.value,
