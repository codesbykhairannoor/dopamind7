  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        ...
        ref: containerRef
      }, _attrs))}><span class="${ssrRenderClass([getStatusOption(__props.modelValue).colorClass, "px-3 py-1.5 rounded-lg text-xs font-bold transition-all border group-hover:shadow-sm"])}">${ssrInterpolate(_ctx.$t(getStatusOption(__props.modelValue).label, __props.modelValue))}</span>`);
      if (isOpen.value) {
        _push(`<div class="fixed sm:absolute top-1/2 sm:top-full left-1/2 sm:left-3 -translate-x-1/2 sm:translate-x-0 -translate-y-1/2 sm:translate-y-0 mt-0 sm:mt-2 w-64 sm:w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 z-[100]"><div class="sm:hidden fixed inset-0 bg-slate-900/40 backdrop-blur-sm -z-10"></div><!--[-->`);
        ssrRenderList(statusOptions, (opt) => {
          _push(`<div class="${ssrRenderClass([__props.modelValue === opt.value ? "bg-slate-50 text-indigo-600" : "text-slate-600", "px-3 py-2.5 rounded-xl text-sm font-bold cursor-pointer hover:bg-slate-50 transition-colors flex items-center gap-3"])}"><span class="w-3 h-3 rounded-full flex-shrink-0" style="${ssrRenderStyle({ backgroundColor: opt.hex })}"></span><span class="truncate">${ssrInterpolate(_ctx.$t(opt.label, opt.value))}</span></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<!---->`);
