import { onMounted, resolveComponent, withCtx, unref, createTextVNode, createVNode, toDisplayString, withDirectives, createBlock, createCommentVNode, vModelText, openBlock, Fragment, renderList, vModelSelect, useSSRContext } from "vue";
      ...
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<div class="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700" data-v-17dccbea${_scopeId}><div class="flex items-center justify-between pb-8 border-b border-slate-200 dark:border-slate-800" data-v-17dccbea${_scopeId}><div data-v-17dccbea${_scopeId}><h2 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight" data-v-17dccbea${_scopeId}>${ssrInterpolate(isEditing ? "Edit Knowledge Transmission" : "Draft New Stream")}</h2><p class="text-slate-500 dark:text-slate-400 text-sm mt-3 font-medium italic" data-v-17dccbea${_scopeId}>Configure content parameters for global broadcast</p></div><div class="flex gap-3" data-v-17dccbea${_scopeId}>`);
            ...
            _push2(`<button class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-lg shadow-indigo-100 dark:shadow-none flex items-center gap-2 active:scale-95" data-v-17dccbea${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$2, {
              name: "sparkles",
              size: "14"
            }, null, _parent2, _scopeId));
            _push2(` ${ssrInterpolate(isEditing ? "Push Changes" : "Publish Stream")}</button></div></div><div class="grid grid-cols-12 gap-8" data-v-17dccbea${_scopeId}><div class="col-span-12 lg:col-span-8 space-y-6" data-v-17dccbea${_scopeId}><div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden p-10 space-y-8" data-v-17dccbea${_scopeId}><div class="space-y-3" data-v-17dccbea${_scopeId}><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1 italic" data-v-17dccbea${_scopeId}>Headline Title</label><input${ssrRenderAttr("value", unref(form).title)} type="text" placeholder="Post title..." class="w-full bg-slate-50 dark:bg-slate-900 border-2 border-transparent rounded-2xl p-5 text-slate-900 dark:text-white text-xl font-bold focus:border-indigo-500/20 focus:ring-4 focus:ring-indigo-500/5 transition-all" required data-v-17dccbea${_scopeId}>`);
            if (unref(form).errors.title) {
              _push2(`<div class="text-rose-500 text-[10px] font-bold uppercase mt-1" data-v-17dccbea${_scopeId}>${ssrInterpolate(unref(form).errors.title)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div class="space-y-3" data-v-17dccbea${_scopeId}><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1 italic" data-v-17dccbea${_scopeId}>Content Engine (Markdown Enabled)</label><textarea rows="18" placeholder="Begin writing..." class="w-full bg-slate-50 dark:bg-slate-900 border-2 border-transparent rounded-2xl p-8 text-slate-700 dark:text-slate-300 font-medium leading-relaxed focus:border-indigo-500/20 focus:ring-4 focus:ring-indigo-500/5 transition-all custom-scrollbar" required data-v-17dccbea${_scopeId}>${ssrInterpolate(unref(form).content)}</textarea>`);
            if (unref(form).errors.content) {
              _push2(`<div class="text-rose-500 text-[10px] font-bold uppercase mt-1" data-v-17dccbea${_scopeId}>${ssrInterpolate(unref(form).errors.content)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div></div><div class="col-span-12 lg:col-span-4 space-y-6" data-v-17dccbea${_scopeId}><div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 space-y-6" data-v-17dccbea${_scopeId}><h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-700/50 pb-4 italic" data-v-17dccbea${_scopeId}>Publish Parameters</h4><div class="space-y-3" data-v-17dccbea${_scopeId}><label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic block" data-v-17dccbea${_scopeId}>Category Classification</label><select class="w-full bg-slate-50 dark:bg-slate-900 border-2 border-transparent rounded-xl p-4 text-slate-700 dark:text-slate-300 text-xs font-bold focus:border-indigo-500/20 focus:ring-4 focus:ring-indigo-500/5 transition-all uppercase tracking-wider" data-v-17dccbea${_scopeId}><!--[-->`);
            ssrRenderList(__props.categories, (cat) => {
              _push2(`<option${ssrRenderAttr("value", cat.id)} data-v-17dccbea${ssrIncludeBooleanAttr(Array.isArray(unref(form).category_id) ? ssrLooseContain(unref(form).category_id, cat.id) : ssrLooseEqual(unref(form).category_id, cat.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(cat.name)}</option>`);
            });
            _push2(`<!--]--></select></div><div class="space-y-3 pt-2" data-v-17dccbea${_scopeId}><div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700/50" data-v-17dccbea${_scopeId}><span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic" data-v-17dccbea${_scopeId}>Visibilty: Live</span><button type="button" class="${ssrRenderClass([unref(form).is_published ? "bg-indigo-600" : "bg-slate-300 dark:bg-slate-700", "w-12 h-6 rounded-full relative transition-all duration-300"])}" data-v-17dccbea${_scopeId}><div class="absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-300 shadow-md" style="${ssrRenderStyle(unref(form).is_published ? "left: calc(100% - 1.25rem)" : "left: 0.25rem")}" data-v-17dccbea${_scopeId}></div></button></div></div></div><div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 space-y-6" data-v-17dccbea${_scopeId}><h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-700/50 pb-4 italic" data-v-17dccbea${_scopeId}>SEO Optimization</h4><div class="space-y-2" data-v-17dccbea${_scopeId}><label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic block" data-v-17dccbea${_scopeId}>Meta Header</label><input${ssrRenderAttr("value", unref(form).meta_title)} type="text" placeholder="Title for Search Engines..." class="w-full bg-slate-50 dark:bg-slate-900 border border-transparent rounded-xl p-4 text-slate-700 dark:text-slate-300 text-xs font-medium focus:border-indigo-500/20 transition-all shadow-inner" data-v-17dccbea${_scopeId}></div><div class="space-y-2" data-v-17dccbea${_scopeId}><label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic block" data-v-17dccbea${_scopeId}>Neural Excerpt</label><textarea rows="2" placeholder="Brief summary for indexing..." class="w-full bg-slate-50 dark:bg-slate-900 border border-transparent rounded-xl p-4 text-slate-700 dark:text-slate-300 text-xs font-medium focus:border-indigo-500/20 transition-all shadow-inner" data-v-17dccbea${_scopeId}>${ssrInterpolate(unref(form).excerpt)}</textarea></div></div></div></div></div>`);
          } else {
            return [
...
const Form = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-17dccbea"]]);
export {
  Form as default
};
