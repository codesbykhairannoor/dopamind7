...
import HabitBatchModal from "./HabitBatchModal-DZDmHnoM.js";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        ...
        if (__props.showCreateModal) {
          _push2(`<div class="fixed inset-0 z-50 flex items-center justify-center p-4"><div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm"></div><div class="bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-lg relative z-10 shadow-2xl animate-in zoom-in-95"><div class="flex justify-between items-center mb-6"><div><h3 class="text-xl md:text-2xl font-black text-slate-800">${ssrInterpolate(__props.isEditing ? "Edit Habit" : _ctx.$t("habit_modal_title"))}</h3>`);
          if (!__props.isEditing) {
            _push2(`<button type="button" class="text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition flex items-center gap-1.5 active:scale-95 w-fit border border-indigo-100 mt-2"><span>⚡</span> Batch Mode </button>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`</div><button class="bg-slate-100 w-8 h-8 rounded-full text-slate-500 hover:bg-slate-200 flex items-center justify-center transition -mt-6">✕</button></div><form class="space-y-5"><div><label class="text-xs font-bold text-slate-400 uppercase mb-2 block">${ssrInterpolate(_ctx.$t("habit_modal_question"))}</label><input${ssrRenderAttr("value", __props.form.name)} class="w-full text-base md:text-lg font-bold py-3 px-4 rounded-xl border-2 border-slate-100 focus:border-indigo-500 transition" placeholder="Contoh: Lari Pagi" required></div><div><label class="text-xs font-bold text-slate-400 uppercase mb-2 block">${ssrInterpolate(_ctx.$t("habit_modal_icon"))}</label><div class="grid grid-cols-6 gap-2 bg-slate-50 p-2 rounded-xl border border-slate-100 max-h-32 md:max-h-40 overflow-y-auto custom-scrollbar"><!--[-->`);
