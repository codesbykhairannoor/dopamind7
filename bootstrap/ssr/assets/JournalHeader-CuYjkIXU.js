...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative z-[60] px-4 py-4 transition-all bg-white border-b shadow-sm border-slate-200 sm:px-6 lg:px-8 md:sticky md:top-0" }, _attrs))}><div class="flex flex-col items-center justify-between w-full gap-4 md:flex-row"><div class="flex items-center w-full gap-4 md:w-auto"><div class="flex items-center justify-center text-white shadow-lg w-12 h-12 bg-indigo-600 rounded-xl shadow-indigo-200 shrink-0">`);
      _push(ssrRenderComponent(_sfc_main$1, {
        name: "journal",
        size: "28"
      }, null, _parent));
      _push(`</div><div><h2 class="text-xl md:text-2xl font-black leading-tight tracking-tight text-slate-800">${ssrInterpolate(_ctx.$t("journal_title", "My Journal"))}</h2><p class="mt-0.5 text-sm font-medium capitalize text-slate-500">${ssrInterpolate(todayDisplay.value)}</p></div></div><div class="flex items-center w-full gap-3 md:w-auto"><div class="hidden lg:flex items-center gap-5 px-5 border-x border-slate-200 h-10"><div class="flex flex-col justify-center"><span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">${ssrInterpolate(_ctx.$t("journal_tasks", "Tugas"))}</span><span class="text-sm font-black text-slate-700 leading-none">${ssrInterpolate(__props.synergy.tasks_completed)}/${ssrInterpolate(__props.synergy.tasks_total)}</span></div><div class="flex flex-col justify-center"><span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">${ssrInterpolate(_ctx.$t("journal_habit", "Habit"))}</span><span class="text-sm font-black text-slate-700 leading-none">${ssrInterpolate(__props.synergy.habits_completed)}</span></div><div class="flex flex-col justify-center"><span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">${ssrInterpolate(_ctx.$t("journal_expense", "Keluar"))}</span><span class="text-sm font-black text-slate-700 leading-none font-mono">${ssrInterpolate(unref(formatMoney)(__props.synergy.expense_total))}</span></div></div>`);
