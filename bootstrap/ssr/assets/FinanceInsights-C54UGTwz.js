...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mt-6 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700" }, _attrs))} data-v-5fd2e58f><div class="bg-indigo-900 rounded-[2.5rem] p-1 shadow-2xl shadow-indigo-200/50 relative overflow-hidden group" data-v-5fd2e58f><div class="absolute inset-0 bg-gradient-to-br from-indigo-600 to-indigo-900" data-v-5fd2e58f></div><div class="absolute -right-10 -top-10 w-40 h-40 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-all duration-1000" data-v-5fd2e58f></div><div class="relative z-10 p-6" data-v-5fd2e58f><div class="flex justify-between items-start mb-6" data-v-5fd2e58f><div data-v-5fd2e58f><div class="flex items-center gap-2 mb-1" data-v-5fd2e58f><span class="text-lg" data-v-5fd2e58f>🧪</span><p class="text-indigo-200 text-[10px] font-black uppercase tracking-[0.2em]" data-v-5fd2e58f>Investment Lab</p></div><h4 class="text-2xl font-black text-white tracking-tighter" data-v-5fd2e58f>${ssrInterpolate(unref(formatMoney)(portfolioSummary.value.currentValue))}</h4><p class="${ssrRenderClass([portfolioSummary.value.totalReturn >= 0 ? "text-emerald-300" : "text-rose-300", "text-[10px] font-bold mt-1"])}" data-v-5fd2e58f>${ssrInterpolate(portfolioSummary.value.totalReturn >= 0 ? "+" : "")}${ssrInterpolate(unref(formatMoney)(portfolioSummary.value.totalReturn))} (Total P/L) </p></div><button class="w-10 h-10 rounded-2xl bg-white/10 hover:bg-white/20 backdrop-blur-md flex items-center justify-center text-xl text-white border border-white/10 transition-all active:scale-95 shadow-lg" data-v-5fd2e58f> ＋ </button></div>`);
      if (assets.value.length > 0) {
        _push(`<div class="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-1" data-v-5fd2e58f><!--[-->`);
        ssrRenderList(assets.value, (asset) => {
          _push(`<div class="bg-indigo-800/40 border border-indigo-500/20 rounded-2xl p-3 flex items-center justify-between group/card hover:bg-indigo-800/60 transition-colors" data-v-5fd2e58f><div class="flex-1 min-w-0 mr-3" data-v-5fd2e58f><div class="flex items-center gap-2 mb-1" data-v-5fd2e58f><h5 class="text-xs font-black text-white truncate" data-v-5fd2e58f>${ssrInterpolate(asset.name)}</h5><span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-white/10 text-white/70" data-v-5fd2e58f>${ssrInterpolate(unref(formatMoney)(asset.capital))}</span></div><div class="flex items-center gap-2" data-v-5fd2e58f><input type="number"${ssrRenderAttr("value", asset.percent)} class="w-10 bg-transparent border-b border-white/20 text-[10px] font-black text-center text-yellow-300 focus:outline-none focus:border-yellow-300 p-0" data-v-5fd2e58f><span class="text-[9px] text-indigo-300" data-v-5fd2e58f>% Growth</span></div></div><div class="text-right" data-v-5fd2e58f><p class="${ssrRenderClass([asset.percent >= 0 ? "text-emerald-400" : "text-rose-400", "text-sm font-black tracking-tight mb-1"])}" data-v-5fd2e58f>${ssrInterpolate(unref(formatMoney)(asset.capital * (1 + asset.percent / 100)))}</p><button class="text-[9px] font-bold text-white bg-rose-500/80 hover:bg-rose-500 px-3 py-1 rounded-lg transition-all shadow-md active:scale-95 opacity-60 group-hover/card:opacity-100" data-v-5fd2e58f> QUIT </button></div></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<div class="py-8 text-center border-2 border-dashed border-white/10 rounded-3xl opacity-50" data-v-5fd2e58f><p class="text-[10px] font-bold text-white uppercase tracking-widest" data-v-5fd2e58f>Lab Kosong</p></div>`);
      }
      _push(`</div></div></div>`);
    };
  }
...
const FinanceInsights = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-5fd2e58f"]]);
