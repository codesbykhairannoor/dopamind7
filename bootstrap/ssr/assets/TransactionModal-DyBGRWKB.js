    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        if (__props.show) {
          ...
          _push2(`<!--]--></select><div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">▼</div></div></div><div class="relative w-full"><label class="block text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1.5">${ssrInterpolate(_ctx.$t("date"))}</label><button type="button" class="${ssrRenderClass([showDatePicker.value ? "ring-2 ring-indigo-500 bg-white" : "", "w-full px-4 py-3.5 rounded-2xl bg-slate-50 hover:bg-slate-100 text-left font-bold text-slate-700 text-sm transition-all flex items-center justify-between"])}"><span class="truncate">${ssrInterpolate(dateDisplay.value)}</span><span class="text-slate-400 text-xs">📅</span></button>`);
          if (showDatePicker.value) {
            _push2(`<div class="absolute bottom-full right-0 sm:right-0 mb-2 z-50 shadow-2xl rounded-3xl origin-bottom sm:origin-bottom-right">`);
          ...
          _push2(`</div></div><div class="flex gap-3 pt-4"><button type="button" class="flex-1 py-3.5 text-slate-500 font-bold hover:bg-slate-50 rounded-2xl transition">${ssrInterpolate(_ctx.$t("btn_cancel"))}</button><button type="submit"${ssrIncludeBooleanAttr(__props.form.processing) ? " disabled" : ""} class="${ssrRenderClass([__props.form.type === "expense" ? "bg-rose-500 hover:bg-rose-600 shadow-rose-200" : "bg-emerald-500 hover:bg-emerald-600 shadow-emerald-200", "flex-[2] py-3.5 text-white font-bold rounded-2xl shadow-lg active:scale-95 transition-all flex justify-center items-center gap-2 disabled:opacity-50"])}">${ssrInterpolate(__props.form.processing ? "..." : _ctx.$t("btn_save"))}</button></div></form></div></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
