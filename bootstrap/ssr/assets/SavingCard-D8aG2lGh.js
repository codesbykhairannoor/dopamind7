...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</button></div></div><div class="p-6 flex flex-col flex-1"><div class="mb-4"><h3 class="text-lg font-black text-slate-800 dark:text-white truncate mb-1">${ssrInterpolate(__props.saving.title)}</h3><p class="text-[10px] font-bold text-slate-400 tracking-wide">${ssrInterpolate(progress.value)}% of ${ssrInterpolate(unref(formatMoney)(__props.saving.target_amount))}</p></div><div class="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full mb-6 overflow-hidden p-0.5"><div class="h-full rounded-full transition-all duration-1000 ease-out shadow-sm" style="${ssrRenderStyle({ width: progress.value + "%", backgroundColor: __props.saving.color || "#6366f1" })}"></div></div><div class="grid grid-cols-2 gap-4 mb-8"><div class="flex flex-col"><span class="text-[8px] font-bold text-slate-400 tracking-tighter">Current</span><span class="text-sm font-black text-slate-800 dark:text-white tabular-nums">${ssrInterpolate(unref(formatMoney)(__props.saving.current_amount))}</span></div><div class="flex flex-col text-right"><span class="text-[8px] font-bold text-slate-400 tracking-tighter">Left to reach</span><span class="text-sm font-black text-rose-500 dark:text-rose-400 tabular-nums">${ssrInterpolate(unref(formatMoney)(remaining.value))}</span></div></div><div class="mt-auto grid grid-cols-2 gap-3"><button class="flex items-center justify-center gap-2 py-3 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[10px] font-bold tracking-widest hover:bg-indigo-600 hover:text-white transition-all active:scale-95">`);
      _push(ssrRenderComponent(unref(ArrowDownCircle), { size: 14 }, null, _parent));
      _push(` Deposit </button><button class="flex items-center justify-center gap-2 py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-bold tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-95">`);
