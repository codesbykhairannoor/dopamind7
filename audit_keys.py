...
def check_missing_keys(json_path, blade_keys):
    ...
    missing = [k for k in blade_keys if k not in json_keys and ':' not in k] # Exclude dynamic keys like 'atomic_faq_q$i'
    return missing
...
if __name__ == "__main__":
    ...
    print(f"Total keys found in blade files: {len(keys)}")
    
    en_missing = check_missing_keys(en_json, keys)
    id_missing = check_missing_keys(id_json, keys)
    
    print(f"Missing in en.json: {en_missing}")
    print(f"Missing in id.json: {id_missing}")
