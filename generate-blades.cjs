...
const apps = [
  { id: 'spreadsheet', name: 'Spreadsheet', type: 'custom' },
  { id: 'notion', name: 'Notion', type: 'custom' },
  { id: 'obsidian', name: 'Obsidian', type: 'custom' },
  { id: 'monday', name: 'Monday.com', type: 'pm' },
  { id: 'trello', name: 'Trello', type: 'pm' },
  { id: 'asana', name: 'Asana', type: 'pm' }
];

const template = (app) => `@extends('layouts.marketing')

@section('title', __('${app.id}_alt_title'))

@section('meta')
    <meta name="description" content="{{ __('${app.id}_alt_desc') }}">
    <meta property="og:title" content="{{ __('${app.id}_alt_og_title') }}">
    <meta property="og:description" content="{{ __('${app.id}_alt_og_desc') }}">
@endsection
...
@section('content')
    ...
    <header class="pt-32 pb-24 px-6 bg-slate-900 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto text-center relative z-10">
            ...
            <h1 class="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-6 tracking-tight leading-tight max-w-5xl mx-auto">
                {{ __('${app.id}_hero_title_1') }} <br/>
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-indigo-600">{{ __('${app.id}_hero_title_2') }}</span>
            </h1>
            
            <p class="text-xl text-indigo-100/80 mb-10 max-w-2xl mx-auto leading-relaxed">
                {!! __('${app.id}_hero_desc') !!}
            </p>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <span class="text-sm text-slate-400 font-medium">{{ __('${app.id}_hero_note') }}</span>
            </div>
        </div>
    </header>
        ...
        <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
            <div>
                <h2 class="text-3xl md:text-5xl font-black text-slate-900 mb-6 leading-tight">
                    {{ __('${app.id}_prob_title_1') }} <span class="text-indigo-600">{{ __('${app.id}_prob_title_highlight') }}</span>
                </h2>
                <p class="text-lg text-slate-600 mb-8">{{ __('${app.id}_prob_desc') }}</p>
                <ul class="space-y-4">
                    <li class="flex items-center gap-3 text-slate-700 font-medium">
                        <div class="w-6 h-6 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-xs">✕</div>
                        {{ __('${app.id}_prob_point_1') }}
                    </li>
                    <li class="flex items-center gap-3 text-slate-700 font-medium">
                        <div class="w-6 h-6 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-xs">✕</div>
                        {{ __('${app.id}_prob_point_2') }}
                    </li>
                    <li class="flex items-center gap-3 text-slate-700 font-medium">
                        <div class="w-6 h-6 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-xs">✕</div>
                        {{ __('${app.id}_prob_point_3') }}
                    </li>
                </ul>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="text-center max-w-3xl mx-auto mb-16">
                <h2 class="text-3xl md:text-5xl font-black text-slate-900 mb-6">
                    {{ __('${app.id}_sol_title_1') }} <span class="text-indigo-600">{{ __('${app.id}_sol_title_highlight') }}</span>
                </h2>
                <p class="text-xl text-slate-600">{{ __('${app.id}_sol_desc') }}</p>
            </div>
            <div class="grid md:grid-cols-2 gap-8">
                <div class="bg-white p-8 rounded-3xl shadow-sm border border-indigo-100 hover:shadow-md transition-shadow">
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 mb-3">{{ __('${app.id}_sol_1_title') }}</h3>
                    <p class="text-slate-600">{{ __('${app.id}_sol_1_desc') }}</p>
                </div>
                <div class="bg-white p-8 rounded-3xl shadow-sm border border-indigo-100 hover:shadow-md transition-shadow">
                    <div class="w-12 h-12 bg-indigo-600 text-white rounded-xl flex items-center justify-center text-xl mb-6 shadow-indigo-200 shadow-lg">⚡</div>
                    <h3 class="text-2xl font-bold text-slate-900 mb-3">{{ __('${app.id}_sol_2_title') }}</h3>
                    <p class="text-slate-600">{{ __('${app.id}_sol_2_desc') }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-24 bg-white">
        <div class="max-w-5xl mx-auto px-6">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-black text-slate-900 mb-4">{{ __('${app.id}_compare_title') }}</h2>
                <p class="text-slate-500">{{ __('${app.id}_compare_desc') }}</p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr>
                            ...
                            <th class="p-4 border-b-2 border-slate-100 text-slate-400 font-medium w-1/3">{{ __('${app.id}_table_head_2') }}</th>
                    ...
                    <tbody class="text-slate-700">
                        <tr class="group hover:bg-slate-50 transition-colors">
                            <td class="p-4 border-b border-slate-100 font-semibold">{{ __('${app.id}_table_row_1_title') }}</td>
                            <td class="p-4 border-b border-slate-100 text-slate-500">{{ __('${app.id}_table_row_1_col_1') }}</td>
                            <td class="p-4 border-b border-indigo-100 font-bold text-indigo-700 bg-indigo-50/50">
                                <div class="flex items-center gap-2">
                                    <span class="text-indigo-500">✓</span> {{ __('${app.id}_table_row_1_col_2') }}
                                </div>
                            </td>
                        </tr>
                        <tr class="group hover:bg-slate-50 transition-colors">
                            <td class="p-4 border-b border-slate-100 font-semibold">{{ __('${app.id}_table_row_2_title') }}</td>
                            <td class="p-4 border-b border-slate-100 text-slate-500">{{ __('${app.id}_table_row_2_col_1') }}</td>
                            <td class="p-4 border-b border-indigo-100 font-bold text-indigo-700 bg-indigo-50/50">
                                <div class="flex items-center gap-2">
                                    <span class="text-indigo-500">✓</span> {{ __('${app.id}_table_row_2_col_2') }}
                                </div>
                            </td>
                        </tr>
                        <tr class="group hover:bg-slate-50 transition-colors">
                            <td class="p-4 border-b border-slate-100 font-semibold">{{ __('${app.id}_table_row_3_title') }}</td>
                            <td class="p-4 border-b border-slate-100 text-slate-500">{{ __('${app.id}_table_row_3_col_1') }}</td>
                            <td class="p-4 border-b border-indigo-100 font-bold text-indigo-700 bg-indigo-50/50 rounded-b-xl">
                                <div class="flex items-center gap-2">
                                    <span class="text-indigo-500">✓</span> {{ __('${app.id}_table_row_3_col_2') }}
                                </div>
                            </td>
                        </tr>
                    </tbody>
        ...
        <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
            <div class="order-2 md:order-1">
                <div class="p-8 bg-slate-800 rounded-3xl border border-slate-700">
                    <h3 class="text-2xl font-bold mb-4 text-indigo-400">{{ __('${app.id}_psycho_title') }}</h3>
                    <p class="text-slate-300 leading-relaxed">{{ __('${app.id}_psycho_desc') }}</p>
                </div>
            </div>
            <div class="order-1 md:order-2">
                <h2 class="text-3xl md:text-5xl font-black mb-6">{{ __('${app.id}_data_title') }}</h2>
                <p class="text-xl text-slate-400 leading-relaxed">{{ __('${app.id}_data_desc') }}</p>
            </div>
        </div>
    </section>
    ...
    <section class="py-32 bg-white text-center">
        <div class="max-w-3xl mx-auto px-6">
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 mb-6 leading-tight">{!! __('${app.id}_cta_title') !!}</h2>
            <p class="text-xl text-slate-500 mb-10">{{ __('${app.id}_cta_desc') }}</p>
...
apps.forEach(app => {
  fs.writeFileSync('resources/views/compare/' + app.id + '.blade.php', template(app));
});
