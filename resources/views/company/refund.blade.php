...
@section('content')
    ...
    {{-- 🔥 PREMIUM HEADER: THE TRUST PROTOCOL 🔥 --}}
    <header class="relative pt-32 pb-24 px-6 bg-slate-900 overflow-hidden border-b border-white/5">
        ...
        <div class="max-w-7xl mx-auto relative z-10 text-center">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 font-mono text-[10px] mb-8 uppercase tracking-widest backdrop-blur-md">
                🛡️ {{ __('refund_badge') }}
            </div>
            ...
            <p class="text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10 font-medium leading-relaxed">
                {{ __('refund_subtitle') }}
            </p>
            ...
            <div class="flex justify-center gap-2 text-xs font-mono text-slate-500 uppercase tracking-widest pt-8">
                ...
                <span>•</span>
                <span>DOC_ID: RFD-2026-FINAL</span>
            </div>
        </div>
    </header>
            ...
            <aside class="hidden lg:block w-1/4 shrink-0 relative">
                <div class="sticky top-28 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <nav class="space-y-1 font-bold text-[13px] text-slate-500" x-data="{ active: 'guarantee' }">
                        <a href="#guarantee" class="block px-4 py-3 rounded-2xl transition hover:bg-slate-50 hover:text-emerald-600" :class="active === 'guarantee' ? 'bg-emerald-50 text-emerald-600' : ''" @click="active = 'guarantee'">1. Jaminan 14 Hari</a>
                        <a href="#process" class="block px-4 py-3 rounded-2xl transition hover:bg-slate-50 hover:text-emerald-600" :class="active === 'process' ? 'bg-emerald-50 text-emerald-600' : ''" @click="active = 'process'">2. Cara Pengajuan</a>
                        <a href="#cancellation" class="block px-4 py-3 rounded-2xl transition hover:bg-slate-50 hover:text-emerald-600" :class="active === 'cancellation' ? 'bg-emerald-50 text-emerald-600' : ''" @click="active = 'cancellation'">3. Pembatalan</a>
                        <a href="#exclusion" class="block px-4 py-3 rounded-2xl transition hover:bg-slate-50 hover:text-emerald-600" :class="active === 'exclusion' ? 'bg-emerald-50 text-emerald-600' : ''" @click="active = 'exclusion'">4. Pengecualian</a>
                    </nav>
                </div>
            </aside>
            ...
            <main class="w-full lg:w-3/4">
                ...
                <div class="bg-white p-8 md:p-14 rounded-[3.5rem] border border-slate-200 shadow-sm prose prose-lg prose-indigo max-w-none text-slate-600 mb-16">
                    <p class="lead text-2xl text-slate-800 font-medium mb-12 leading-relaxed italic">
                        "{{ __('refund_intro_text') }}"
                    </p>

                    <h2 id="guarantee" class="scroll-mt-32 font-black text-slate-900 text-4xl mb-8">{{ __('refund_h2_guarantee') }}</h2>
                    <p>{{ __('refund_p_guarantee') }}</p>
                    <ul class="marker:text-emerald-600 font-bold">
                        <li>{{ __('refund_li_guarantee_1') }}</li>
                        <li>{{ __('refund_li_guarantee_2') }}</li>
                    </ul>

                    <div class="my-10 p-8 bg-emerald-50 rounded-3xl border border-emerald-100 flex gap-6 items-start">
                        <span class="text-3xl mt-1">🧪</span>
                        <div class="not-prose">
                            <h4 class="text-emerald-900 font-black text-lg mb-2">Uji Coba Tanpa Risiko</h4>
                            <p class="text-emerald-800/80 text-sm leading-relaxed">Kami percaya pada kualitas OneForMind. Jika Anda merasa sistem kami tidak membantu habit atau keuangan Anda dalam 2 minggu pertama, kami kembalikan dana Anda 100%.</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-200 not-prose mb-12 shadow-inner">
                        <div class="space-y-6">
                            <div class="flex items-center gap-6">
                                <span class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center font-black text-slate-400 border border-slate-200 decoration-none">01</span>
                                <p class="m-0 text-slate-800 font-black">{{ __('refund_li_process_1') }}</p>
                            </div>
                            <div class="flex items-center gap-6">
                                <span class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center font-black text-slate-400 border border-slate-200">02</span>
                                <p class="m-0 text-slate-800 font-black">{{ __('refund_li_process_2') }}</p>
                            </div>
                            <div class="flex items-center gap-6">
                                <span class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center font-black text-slate-400 border border-slate-200">03</span>
                                <p class="m-0 text-slate-800 font-black">{{ __('refund_li_process_3') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-8 bg-rose-50 rounded-3xl border border-rose-100 text-rose-900 text-sm leading-relaxed font-bold">
                        ⚠️ {{ __('refund_p_exclusion') }}
                    </div>
                </div>

                {{-- FINAL CTA BENTO --}}
                <div class="bg-emerald-600 rounded-[4rem] p-12 md:p-20 text-white shadow-2xl relative overflow-hidden group mb-12">
                     ...
                     <div class="relative z-10 text-center">
                         ...
                         <p class="text-emerald-100 text-xl mb-12 max-w-xl mx-auto opacity-80">
                             {{ __('refund_cta_desc') }}
                         </p>
                         <a href="mailto:oneformindapp@gmail.com" class="inline-flex items-center gap-3 px-12 py-5 bg-white text-emerald-600 rounded-full font-black text-xl hover:shadow-2xl hover:-translate-y-1 transition active:scale-95 shadow-xl">
