...
<template>
    <div class="group relative bg-[#ffffff05] dark:bg-slate-900/40  rounded-[2.5rem] border border-white/10 dark:border-slate-800/50 shadow-sm hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-700 overflow-hidden flex flex-col h-full border-b-4" :style="{ borderBottomColor: (saving.color || '#6366f1') }">
        <!-- Floating Glow -->
        ...
        <div class="p-6 pb-2 flex items-start justify-between relative z-10">
            <div class="w-14 h-14 rounded-2xl bg-white/5 dark:bg-black/20  border border-white/10 dark:border-white/5 flex items-center justify-center text-3xl shadow-xl transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-700">
                {{ saving.icon || '🏦' }}
            </div>
            
            <div class="flex gap-2">
                <button @click="onEdit(saving)" class="w-9 h-9 rounded-full bg-white/10 dark:bg-slate-800/50  flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all transform hover:scale-110 active:scale-95 shadow-sm">
                    <Edit3 :size="15" />
                </button>
                <button @click="onDelete(saving)" class="w-9 h-9 rounded-full bg-white/10 dark:bg-slate-800/50  flex items-center justify-center text-slate-400 hover:text-rose-500 transition-all transform hover:scale-110 active:scale-95 shadow-sm">
                    <Trash2 :size="15" />
                </button>
            </div>
        </div>
