...
<template>
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 items-stretch">
        ...
        <div class="lg:col-span-7 relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-600 via-indigo-500 to-violet-600 dark:from-indigo-700 dark:via-indigo-600 dark:to-violet-700 p-6 md:p-8 text-white shadow-2xl dark:shadow-none shadow-indigo-200/50 dark:shadow-indigo-900/20 flex flex-col justify-center min-h-[200px] transition-all duration-500">
            <div class="relative z-10 flex flex-col h-full justify-between">
                ...
                <div class="mt-6 pt-6 border-t border-white/10 flex items-center gap-3">
                     ...
                     <span class="text-[10px] font-medium text-indigo-100">{{ $t('monthly_finance_desc', 'Your financial performance this month.') }}</span>
                </div>
            </div>
        ...
        <div class="lg:col-span-5 grid grid-rows-[auto_1fr] gap-4">
                ...
                <div class="flex-1 min-w-0"> 
                    ...
                    <div v-if="!isEditingSalary" class="flex items-center justify-between">
                        <h4 class="text-xl md:text-2xl font-black text-slate-800 dark:text-white truncate pr-2 transition-colors duration-500">
                            {{ formatMoney(stats?.income_target || 0) }}
                        </h4>
                        <button @click="startEditing" class="w-8 h-8 flex items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-600 hover:bg-indigo-600 hover:text-white transition-all transform scale-90 opacity-0 group-hover:scale-100 group-hover:opacity-100 shadow-sm dark:shadow-none transition-all duration-300">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        </button>
                    </div>

                    <div v-else class="flex items-center gap-2 w-full animate-in zoom-in-95 duration-200 mt-1">
                        <input 
                            ref="inputSalaryRef"
                            v-model="displaySalary"
                            type="text" 
                            class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-3 py-1.5 text-xl font-black text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600"
            ...
            <div class="grid grid-cols-2 gap-4 h-full min-h-[160px]">
                <div class="bg-emerald-50/40 dark:bg-emerald-500/5 border border-emerald-100/60 dark:border-emerald-500/20 rounded-[2rem] p-5 flex flex-col justify-center relative overflow-hidden group hover:bg-emerald-50 dark:hover:bg-emerald-500/10 shadow-sm dark:shadow-none transition-colors duration-500">
                    ...
                    <h4 class="text-lg md:text-xl font-black text-emerald-700 dark:text-emerald-400 truncate transition-colors duration-500">
                        {{ formatMoney(stats?.total_income || 0) }}
                    </h4>
                </div>
                ...
                <div class="bg-rose-50/40 dark:bg-rose-500/5 border border-rose-100/60 dark:border-rose-500/20 rounded-[2rem] p-5 flex flex-col justify-center relative overflow-hidden group hover:bg-rose-50 dark:hover:bg-rose-500/10 shadow-sm dark:shadow-none transition-colors duration-500">
                    ...
                    <h4 class="text-lg md:text-xl font-black text-rose-700 dark:text-rose-400 truncate transition-colors duration-500">
                        {{ formatMoney(stats?.total_expense || 0) }}
                    </h4>
                </div>
            </div>
