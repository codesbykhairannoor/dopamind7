...
const showDatePicker = ref(false); // State untuk kalender
...
// Format tampilan tanggal di header
...
<template>
    <Modal :show="show" @close="close" maxWidth="2xl">
        <div class="bg-white rounded-[2.5rem] flex flex-col max-h-[90vh] relative overflow-visible transition-all duration-300 border border-slate-100 shadow-2xl">
            
            <div v-if="conflictError" class="absolute top-0 left-0 right-0 bg-rose-500 text-white text-[10px] font-black px-6 py-3.5 text-center animate-in slide-in-from-top-full z-50 shadow-lg flex items-center justify-center gap-2 uppercase tracking-[0.1em] rounded-t-[2.5rem]">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/30 p-4 md:p-8">
                ...
                <div class="hidden md:grid grid-cols-12 gap-2 mb-4 px-1 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    <div class="col-span-2 text-center">{{ $t('type', 'Tipe') }}</div>
                    <div class="col-span-3 ml-1">{{ $t('category', 'Kategori') }} <span class="text-rose-500">*</span></div>
                    <div class="col-span-4 ml-1">{{ $t('description', 'Keterangan') }} <span class="text-rose-500">*</span></div>
                    <div class="col-span-2">{{ $t('amount', 'Nominal') }} <span class="text-rose-500">*</span></div>
                ...
                <div class="space-y-4 md:space-y-3 mt-2">
                    <div v-for="(trx, index) in form.transactions" :key="index" 
                        ...
                        <div class="grid grid-cols-12 gap-3 md:gap-2 items-center">
                            
                            <div class="col-span-12 md:col-span-2">
                                <label class="md:hidden text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1 block">{{ $t('type', 'Tipe') }}</label>
                                <div class="flex bg-slate-100 p-1 rounded-[0.8rem] h-11">
                                    <button type="button" @click="changeType(index, 'expense')" class="flex-1 rounded-lg text-[10px] font-black transition-all flex items-center justify-center tracking-wider uppercase" :class="trx.type === 'expense' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                                        Out
                                    </button>
                                    <button type="button" @click="changeType(index, 'income')" class="flex-1 rounded-lg text-[10px] font-black transition-all flex items-center justify-center tracking-wider uppercase" :class="trx.type === 'income' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                                        In
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="col-span-12 md:col-span-3">
                                <label class="md:hidden text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1 block">{{ $t('category', 'Kategori') }}</label>
                                <div class="relative">
                                    <select v-model="trx.category" class="w-full pl-3 pr-8 h-11 !rounded-[0.8rem] border-2 border-slate-200 focus:border-indigo-500 bg-white font-bold text-slate-700 capitalize text-xs appearance-none cursor-pointer transition-all"
                                        :class="{'!border-rose-300 !bg-rose-50': form.errors[`transactions.${index}.category`]}">
                            ...
                            <div class="col-span-12 md:col-span-4">
                                <label class="md:hidden text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1 block">{{ $t('description', 'Keterangan') }}</label>
                                <TextInput 
                                    v-model="trx.title" 
                                    :placeholder="$t('desc_placeholder', 'Makan, Gaji...')" 
                                    class="w-full text-xs font-bold h-11 px-3 !rounded-[0.8rem] border-2 border-slate-200 focus:border-indigo-500 focus:ring-0 transition-all bg-white" 
                                    :class="{'!border-rose-300 !bg-rose-50': form.errors[`transactions.${index}.title`]}"
                                />
                            </div>

                            <div class="col-span-10 md:col-span-2">
                                <label class="md:hidden text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1 block">{{ $t('amount', 'Nominal') }}</label>
                                <div class="relative group">
                                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-[10px]">{{ currencySymbol }}</span>
                                    <input type="text" :value="formatDisplay(trx.amount)" @input="e => onInputAmount(e, index)" placeholder="0" 
                                        class="w-full h-11 pl-8 pr-2 !rounded-[0.8rem] border-2 border-slate-200 focus:border-indigo-500 font-black text-xs text-slate-700 transition-all focus:ring-0 bg-white"
                                        :class="{'!border-rose-300 !bg-rose-50': form.errors[`transactions.${index}.amount`]}"
                                    >
                                </div>
                            </div>

                            <div class="col-span-2 md:col-span-1 flex justify-center items-end md:items-center h-11 md:h-auto">
                                <button @click="removeRow(index)" type="button" 
                                    class="w-full h-11 md:w-8 md:h-8 rounded-xl flex items-center justify-center text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-all active:scale-90"
                                    :disabled="form.transactions.length <= 1"
                                    :class="{'opacity-0 pointer-events-none': form.transactions.length <= 1}">
                                    <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                </button>
                            </div>
                ...
                <button @click="addRow" type="button" class="mt-8 w-full py-3.5 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 font-black uppercase tracking-widest text-[10px] hover:border-indigo-400 hover:text-indigo-600 hover:bg-white transition-all flex items-center justify-center gap-3 group active:scale-95 shadow-sm">
                    <span class="w-5 h-5 rounded-md bg-slate-100 text-slate-400 flex items-center justify-center text-xs group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">+</span> 
                    {{ $t('btn_add_another', 'Tambah Baris') }}
                </button>
            </div>
