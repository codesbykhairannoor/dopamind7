    ...
    <div class="relative min-h-screen flex flex-col items-center justify-center bg-white selection:bg-indigo-100 selection:text-indigo-700 p-6 overflow-hidden">
        ...
        <div class="w-full max-w-[420px] flex flex-col relative z-10">
            <div class="flex justify-center mb-6">
                <a :href="route('home')" class="w-12 h-12 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200 dark:shadow-none transition-transform duration-500 hover:rotate-[360deg]">
                    <img src="/favicon.svg?v=2" alt="OneForMind Logo" class="h-8 w-8 brightness-0 invert" />
                </a>
            </div>

            <div class="text-center mb-8">
                <h1 class="text-2xl font-black text-slate-900 mb-1.5 tracking-tight">
                    {{ $t('auth_register_title', 'Seconds to sign up!') }}
                </h1>
                <p class="text-sm font-medium text-slate-500">
                    {{ $t('auth_has_account', 'Already have an account?') }}
                    <Link :href="route('login')" class="text-indigo-600 hover:text-indigo-800 transition font-bold">{{ $t('auth_link_login', 'Sign in') }}</Link>
                </p>
            </div>
            ...
            <button 
                ...
                class="flex items-center justify-center w-full px-4 py-3 border border-indigo-500 rounded-xl shadow-lg shadow-indigo-100 bg-indigo-600 hover:bg-indigo-700 transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-indigo-300 mb-6 group active:scale-[0.98]">
                ...
                <span class="font-bold text-white text-sm">{{ $t('auth_btn_google_up', 'Lanjutkan dengan Google') }}</span>
            </button>

            <div class="relative flex items-center justify-center mb-6">
                <div class="border-t border-slate-200 w-full"></div>
                <span class="bg-white px-3 text-[10px] font-black uppercase text-slate-400 absolute tracking-wider">or</span>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-4">
                <div>
                    <input
                        ...
                        :class="form.errors.name ? 'border-red-400 focus:ring-red-100 focus:border-red-500' : 'focus:border-indigo-500 focus:ring-indigo-500/10'"
                    />
                    <InputError class="mt-1.5 ml-1" :message="form.errors.name" />
                </div>
                ...
                <div class="relative">
                    <input
                        ...
                        class="w-full pl-4 pr-10 py-3 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-4 transition-all outline-none"
                        :class="form.errors.password ? 'border-red-400 focus:ring-red-100 focus:border-red-500' : 'focus:border-indigo-500 focus:ring-indigo-500/10'"
                    />
                ...
                <button
                    ...
                    class="w-full mt-4 bg-indigo-600 text-white font-black py-3 rounded-xl shadow-lg shadow-indigo-100 dark:shadow-none hover:bg-indigo-700 transition transform hover:-translate-y-0.5 active:scale-95 disabled:opacity-75 flex items-center justify-center gap-2"
                >
                    <span>{{ $t('auth_btn_register', 'Sign up with Email') }}</span>
