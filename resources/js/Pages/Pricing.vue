    ...
    return [
        {
            ...
            features: [
                ...
                {
                    text: "pricing_f_advanced_ai_locked",
                    icon: Lock,
                    isLocked: true,
                },
            ],
        ...
        },
        {
            ...
            features: [
                {
                    text: "pricing_feature_header_explorer_plus",
                    isHeader: true,
                },
                { text: "pricing_f_all_tabs_open", icon: ListChecks },
                { text: "pricing_f_unlimited_habit", icon: Zap },
                { text: "pricing_f_the_vault", icon: Wallet },
                { text: "pricing_f_finance_trends", icon: Calendar },
                { text: "pricing_f_batch_planner", icon: ShieldCheck },
                { text: "pricing_f_neural_sync", icon: Cloud },
            ],
        ...
        },
        {
            ...
            features: [
                {
                    text: "pricing_feature_header_architect_plus",
                    isHeader: true,
                },
                { text: "pricing_f_neural_ai", icon: Sparkles },
                { text: "feat_ai_stacking", icon: Brain },
                { text: "feat_ai_audit", icon: Star },
                { text: "feat_sentiment_ai", icon: MessageSquare },
                { text: "feat_resume_ai", icon: ShieldCheck },
        ...
        },
        {
            ...
            features: [
                ...
                { text: "pricing_f_founder_badge", icon: Star },
                { text: "pricing_f_neural_sync", icon: Cloud },
...
<template>
    ...
    <div class="bg-slate-50 dark:bg-slate-950 min-h-screen pb-32">
            ...
            <div class="text-center pt-20 pb-2 px-4 md:px-8">
                <h1
                    class="text-4xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tighter mb-4 mx-auto"
                >
                    {{ $t("pricing_hero_title") }}
                    <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500"
                        >{{ $t("pricing_hero_highlight") }}</span
                    >
                </h1>
                <p
                    class="text-slate-500 dark:text-slate-400 font-bold max-w-xl mx-auto leading-relaxed"
                >
                    {{ $t("pricing_hero_desc") }}
                </p>
                ...
                <div class="flex justify-center mt-8 mb-14">
                    <div
                        class="inline-flex items-center p-1 bg-white dark:bg-slate-900 rounded-[1.8rem] border border-slate-100 dark:border-slate-800 shadow-sm scale-110"
                    >
                        <button
                        ...
                        >
                            ...
                            <span
                                class="absolute -top-4 -right-2 px-2 py-1 rounded-lg bg-emerald-500 text-[8px] text-white animate-bounce shadow-lg"
                                >{{ $t("pricing_billing_save") }}</span
                            >
                        </button>
                    </div>
                </div>
            ...
            <div class="px-4 md:px-8">
                <!-- 💎 Pricing Grid -->
                <div
                    class="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8"
                >
                    <div
                        ...
                        :class="[
                            ...
                            plan.theme === 'dark'
                                ? 'dark:bg-indigo-950 border-indigo-900'
                                : '',
                        ]"
                    >
                        <!-- Badge Header -->
                        ...
                        <p
                            class="text-[11px] font-bold text-slate-500 leading-relaxed min-h-[44px] mb-8"
                        >
                            {{ $t(plan.desc) }}
                        </p>
                        ...
                        <div
                            class="space-y-4 mb-10 flex-grow border-t border-slate-50 dark:border-slate-800 pt-8"
                        >
                            <div
                                v-for="(feat, fIdx) in plan.features"
                                :key="fIdx"
                                class="flex items-start gap-4"
                                :class="feat.isLocked ? 'opacity-40' : ''"
                            >
                                ...
                                <template v-else>
                                    <div
                                        class="mt-1 flex items-center justify-center shrink-0"
                                    >
                                        <component
                                            :is="feat.icon"
                                            :size="14"
                ...
                <div
                    class="mt-40 max-w-5xl mx-auto animate-in slide-in-from-bottom-12 duration-1000"
                >
                    <div class="text-center mb-16">
                        ...
                        <p
                            class="text-xs font-bold text-slate-400 capitalize tracking-widest"
                        >
                    ...
                    <div
                        class="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-2xl"
                    >
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-800/50">
                                    <th
                                        class="p-8 text-[10px] font-black tracking-wide text-slate-400"
                                    >
                                        {{ $t("pricing_feature_col_name") }}
                                    </th>
                                    <th
                            ...
                            >
                                <tr
                                    v-for="row in [
                                        ...
                                        },
                                        {
                                            ...
                                            explorer: '—',
                                            architect: $t('pricing_v_partial'),
                                            quantum: $t('pricing_v_full'),
                                        },
                                    ]"
                                    :key="row.name"
                                ...
                                >
                                    <td
                                        class="p-8 text-xs font-black text-slate-700 dark:text-slate-200"
                                    >
                                        {{ row.name }}
                                    </td>
                                    <td
                                        class="p-8 text-xs font-bold text-slate-400 text-center"
                                    >
                                        {{ row.explorer }}
                                    </td>
                                    <td
                                        class="p-8 text-xs font-black text-indigo-600 dark:text-indigo-400 text-center"
                                    >
                                        {{ row.architect }}
                                    </td>
                                    <td
                                        class="p-8 text-xs font-black text-amber-600 dark:text-amber-500 text-center"
                                    >
                                        {{ row.quantum }}
                                    </td>
                                </tr>
                    ...
                    <div class="mt-16 text-center">
                        <p
                            class="text-[10px] font-black text-slate-400 tracking-wide mb-4"
                        >
                            {{ $t("pricing_infra_badge") }}
                        </p>
                        <div
                            class="flex flex-wrap justify-center items-center gap-12 opacity-30 grayscale contrast-125"
                        >
