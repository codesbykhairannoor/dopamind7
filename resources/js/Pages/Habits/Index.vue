...
<div v-if="showDeleteModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="min-w-0 flex-1">
        ...
        <div class="flex items-center gap-2 mt-1">
            ...
            <div class="hidden md:flex opacity-0 group-hover:opacity-100 transition items-center gap-1 bg-white/80 backdrop-blur-sm px-1 rounded-md absolute right-2 top-1/2 -translate-y-1/2 shadow-sm border border-slate-100">
    ...
    <span v-if="getStatus(habit, day.dateString) === 'skipped'" class="text-[10px] md:text-xs font-bold">-</span>
            ...
            <div v-if="localHabits.length > 0" class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 animate-in slide-in-from-bottom-5 px-4 md:px-0">
                <div class="bg-indigo-600 rounded-2xl p-5 md:p-6 text-white shadow-xl shadow-indigo-200">
                    ...
                    <div class="w-full bg-indigo-800 rounded-full h-2"><div class="bg-white h-2 rounded-full" :style="{ width: overallPercentage + '%' }"></div></div>
                </div>
                <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-slate-100 flex items-center gap-4">
                    ...
                    <div><div class="text-slate-400 text-[10px] md:text-xs font-bold uppercase">{{ $t('stats_total_check') }}</div><div class="text-xl md:text-2xl font-black text-slate-800">{{ totalCompletions }}</div></div>
                </div>
                <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-slate-100 flex items-center gap-4">
                    <div class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center text-xl md:text-2xl">🔥</div>
                    <div><div class="text-slate-400 text-[10px] md:text-xs font-bold uppercase">{{ $t('stats_mood') }}</div><div class="text-lg md:text-xl font-black text-slate-800">{{ $t('stats_mood_desc') }}</div></div>
                </div>
            </div>
