...
<template>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 w-full max-w-sm relative z-10 shadow-2xl dark:shadow-none animate-in zoom-in-95 text-center overflow-hidden border border-slate-100 dark:border-slate-800">
            ...
            <h3 class="text-xl font-black text-slate-800 dark:text-slate-100 mb-2">{{ $t('habit_delete_title') }}</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
                {{ $t('habit_delete_desc') }} <br>
                <span class="font-bold text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">{{ habitToDelete?.name }}</span>?
            </p>
            <div class="flex gap-3">
                <button @click="closeModal" class="flex-1 py-3 rounded-xl font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition">{{ $t('habit_btn_cancel') }}</button>
                <button @click="executeDelete" class="flex-1 py-3 rounded-xl font-bold text-white bg-rose-500 hover:bg-rose-600 shadow-lg shadow-rose-200 dark:shadow-none transition transform hover:-translate-y-0.5">{{ $t('habit_btn_delete_confirm') }}</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showCopyModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 w-full max-w-sm relative z-10 shadow-2xl dark:shadow-none animate-in zoom-in-95 text-center overflow-hidden border border-slate-100 dark:border-slate-800">
            ...
            <h3 class="text-xl font-black text-slate-800 dark:text-slate-100 mb-2">{{ $t('habit_copy_title') }}</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">{{ $t('habit_copy_desc') }}</p>
            <div class="flex gap-3">
                <button @click="closeModal" class="flex-1 py-3.5 rounded-xl font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition">{{ $t('habit_btn_cancel') }}</button>
                <button @click="executeCopy" class="flex-1 py-3.5 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none transition transform hover:-translate-y-0.5">{{ $t('habit_btn_copy_confirm') }}</button>
            </div>
        </div>
    </div>
