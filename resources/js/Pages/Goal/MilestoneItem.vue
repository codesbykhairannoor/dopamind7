...
<template>
    <div class="flex items-center group/ms gap-3 py-1.5 px-3 bg-transparent hover:bg-slate-50/50 rounded-2xl transition-all duration-300 relative border border-transparent hover:border-slate-100"
        ...
        <div class="flex-grow flex flex-col min-w-0">
            <input 
                ...
                class="w-full bg-transparent border-none focus:ring-0 p-0 text-sm placeholder:text-slate-300 transition-all"
                :class="(milestone.is_completed || milestone.completed) ? 'text-slate-400 line-through font-medium' : 'text-slate-700 font-black'"
            />
            
            <div class="flex items-center gap-2 mt-0.5">
                <button @click="showDatePicker = !showDatePicker" class="flex items-center gap-1.5 px-1.5 py-0.5 rounded-md hover:bg-slate-100 transition-colors group/date">
                    <Calendar :size="10" :class="(milestone.is_completed || milestone.completed) ? 'text-slate-300' : 'text-slate-400 group-hover/date:text-indigo-500'" />
                    <span class="text-[9px] font-black uppercase tracking-widest" :class="(milestone.is_completed || milestone.completed) ? 'text-slate-300' : 'text-slate-400 group-hover/date:text-slate-600'">
                        {{ dateDisplay }}
                    </span>
                </button>
            </div>
        ...
        <button 
            @click="emit('delete')"
            class="opacity-0 group-hover/ms:opacity-100 p-1.5 text-slate-300 hover:text-rose-500 transition-all rounded-xl hover:bg-rose-50 focus:outline-none shrink-0"
        >
            <Trash2 :size="14" />
        </button>
        ...
        <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100">
            <div v-if="showDatePicker" class="absolute left-10 top-full mt-2 z-50 bg-white rounded-3xl shadow-2xl border border-slate-100 p-2 animate-in fade-in zoom-in duration-200">
                <GoalDatePicker 
                    :show="true" 
