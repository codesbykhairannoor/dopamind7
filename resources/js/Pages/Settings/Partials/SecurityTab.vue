...
<template>
    <div class="space-y-12">
        ...
        <section class="space-y-6">
            <div>
                <h3 class="text-lg font-black text-slate-800 tracking-tight">Keamanan Akun</h3>
                <p class="text-xs font-bold text-slate-400 mt-1">Pastikan akun Anda tetap aman dengan password yang kuat.</p>
            </div>

            <form @submit.prevent="updatePassword" class="space-y-5">
                <div v-if="hasPassword" class="space-y-1.5">
                    <label class="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Password Saat Ini</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="space-y-1.5">
                        <label class="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Password Baru</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Konfirmasi Password</label>
        ...
        <section class="pt-8 border-t border-slate-100 space-y-6">
            <div class="p-6 rounded-[2rem] bg-rose-50 border border-rose-100">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h4 class="font-black text-rose-900 text-sm italic">Zona Berbahaya</h4>
                        <p class="text-xs font-bold text-rose-600/70">Hapus akun Anda secara permanen. Data tidak bisa dikembalikan.</p>
                    </div>
                </div>
        ...
        <div v-if="showDeleteConfirm" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div class="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl border border-slate-100 animate-in fade-in zoom-in duration-300">
                <h3 class="text-xl font-black text-slate-800 mb-2">Konfirmasi Penghapusan</h3>
                <p class="text-xs font-bold text-slate-500 mb-6 leading-relaxed">Masukkan password Anda untuk mengonfirmasi bahwa Anda benar-benar ingin menghapus akun Oneformind Anda.</p>
                
                <input type="password" v-model="deleteForm.password" class="w-full px-5 py-4 rounded-2xl border-slate-100 bg-slate-50/30 text-slate-700 font-bold mb-6 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none" placeholder="Password Anda...">

                <div class="flex gap-4">
                    <button @click="showDeleteConfirm = false" class="flex-1 px-6 py-4 rounded-2xl bg-slate-100 text-slate-600 font-black text-sm hover:bg-slate-200 transition-all">Batal</button>
                    <button @click="deleteAccount" :disabled="deleteForm.processing" class="flex-1 px-6 py-4 rounded-2xl bg-rose-600 text-white font-black text-sm hover:bg-rose-700 transition-all shadow-lg shadow-rose-200">Hapus Akun</button>
                </div>
            </div>
        </div>
    </div>
