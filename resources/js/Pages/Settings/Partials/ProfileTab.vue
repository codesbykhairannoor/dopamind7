...
<template>
    <div class="space-y-8">
        <div>
            <h3 class="text-lg font-black text-slate-800 dark:text-white tracking-tight transition-colors duration-500">{{ $t('profile_info_title', 'Informasi Profil') }}</h3>
            <p class="text-xs font-bold text-slate-400 dark:text-slate-500 mt-1 transition-colors duration-500">{{ $t('profile_info_desc', 'Perbarui nama, email, dan foto profil Anda.') }}</p>
        </div>

        <form @submit.prevent="updateProfile" class="space-y-6">
            <!-- Avatar Upload -->
            <div class="flex flex-col items-center sm:flex-row gap-6 p-6 bg-slate-50/50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700 transition-colors duration-500">
                <div class="relative group">
                    <img :src="avatarPreview" class="w-24 h-24 rounded-3xl object-cover border-4 border-white dark:border-slate-800 shadow-md dark:shadow-none group-hover:opacity-90 transition-all duration-500" alt="Avatar">
                    <div class="absolute -bottom-2 -right-2 w-8 h-8 rounded-xl bg-indigo-600 text-white flex items-center justify-center shadow-lg dark:shadow-none border-2 border-white dark:border-slate-800 cursor-pointer hover:scale-110 transition-transform">
            ...
            <div class="flex justify-end pt-2">
                <button type="submit" :disabled="form.processing" 
                    class="bg-indigo-600 hover:bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-sm transition-all active:scale-95 shadow-lg dark:shadow-none shadow-indigo-200 disabled:opacity-50">
