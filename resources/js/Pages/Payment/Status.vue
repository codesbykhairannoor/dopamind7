...
<template>
    <Head :title="status === 'success' ? 'Welcome to the Ecosystem!' : (status === 'pending' ? 'Payment Pending' : 'Payment Cancelled')" />
    ...
    <div class="min-h-[80vh] flex items-center justify-center px-4 py-12 bg-slate-50/30 dark:bg-slate-950/20">
            <div class="max-w-xl w-full text-center animate-in fade-in zoom-in-95 duration-1000">
                ...
                <!-- Success Icon -->
                <div v-if="status === 'success'" class="relative inline-block mb-10">
                ...
                <!-- Pending Icon -->
                <div v-else-if="status === 'pending'" class="relative inline-block mb-10">
                    <div class="absolute inset-0 bg-amber-500 blur-3xl opacity-20 scale-150 animate-pulse"></div>
                    <div class="relative w-24 h-24 md:w-32 md:h-32 rounded-[2.5rem] bg-amber-500 flex items-center justify-center text-white shadow-2xl shadow-amber-200 dark:shadow-none hover:scale-105 transition-transform duration-500">
                        <OneForMindIcon name="clock" size="64" />
                    </div>
                </div>

                <!-- Failed Icon -->
                <div v-else class="relative inline-block mb-10">
                    <div class="absolute inset-0 bg-rose-500 blur-3xl opacity-20 scale-150 animate-pulse"></div>
                    <div class="relative w-24 h-24 md:w-32 md:h-32 rounded-[2.5rem] bg-rose-500 flex items-center justify-center text-white shadow-2xl shadow-rose-200 dark:shadow-none -rotate-6 hover:rotate-0 transition-transform duration-500">
                        <OneForMindIcon name="x" size="64" />
                    </div>
                </div>

                <!-- Text Content -->
                <h1 class="text-3xl md:text-5xl font-black text-slate-900 dark:text-white mb-4 tracking-tight leading-tight">
                    <template v-if="status === 'success'">{{ $t('payment_success_title', 'Upgrade Complete!') }}</template>
                    <template v-else-if="status === 'pending'">{{ $t('payment_pending_title', 'Payment Pending') }}</template>
                    <template v-else>{{ $t('payment_failed_title', 'Payment Cancelled') }}</template>
                </h1>
                
                <p v-if="status === 'success'" class="text-lg md:text-xl font-bold text-slate-500 dark:text-slate-400 mb-10">
                ...
                <p v-else-if="status === 'pending'" class="text-lg md:text-xl font-bold text-slate-500 dark:text-slate-400 mb-10">
                    {{ $t('payment_pending_msg', 'Your payment is currently being processed. We will notify you once it is confirmed.') }}
                </p>
                <p v-else class="text-lg md:text-xl font-bold text-slate-500 dark:text-slate-400 mb-10">
                    {{ $t('payment_failed_msg', 'Your payment was cancelled or failed. You can try again anytime.') }}
                </p>

                <!-- Plan Badge (Only on success) -->
                <div v-if="status === 'success'" class="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm mb-12">
                ...
                <div v-if="status === 'success'" class="mt-16 text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] flex items-center justify-center gap-4">
