        ...
        <div class="w-full lg:max-w-[1400px] mx-auto p-4 md:p-8 lg:p-12">
            ...
            <header class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 pt-8">
                <div>
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <p class="text-indigo-500 dark:text-indigo-400 font-black uppercase tracking-[0.3em] text-[9px]">{{ synergy?.date_formatted }}</p>
                    </div>
                ...
                <div class="flex items-center gap-6 bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-sm hover:shadow-xl transition-all duration-700 active:scale-95 group cursor-default ultra-shadow">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1.5">{{ $t('dash_synergy') }}</p>
            ...
            <div class="mb-12 overflow-x-auto no-scrollbar scroll-smooth">
                <div class="flex items-center gap-3 w-max">
                    <Link :href="route('planner.index')" class="flex items-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-lg active:scale-95">
                        <Plus :size="14" stroke-width="3" /> {{ $t('btn_add_task', 'New Task') }}
                    </Link>
                    <Link :href="route('finance.index')" class="flex items-center gap-2 bg-white dark:bg-slate-900 text-slate-600 dark:text-indigo-400 px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-indigo-500/10 transition-all shadow-sm active:scale-95">
                    ...
                    <div v-if="!isExplorer" @click="demandAccess('journal', route('journal.index'))" class="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-indigo-100 dark:border-indigo-500/20 hover:bg-indigo-100 transition-all cursor-pointer shadow-sm active:scale-95">
                ...
                <div class="col-span-12 lg:col-span-8 space-y-8">
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center justify-between mb-10">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ synergy?.planner.total }} Active Protocols</p>
                                    </div>
                                </div>
                            ...
                            <div v-if="synergy?.planner.upcoming.length > 0" class="space-y-4">
                                <div v-for="task in synergy.planner.upcoming" :key="task.id" class="flex items-center justify-between p-5 bg-slate-50/50 dark:bg-white/5 rounded-2xl border border-transparent hover:border-slate-100 dark:hover:border-white/10 transition-all">
                                    ...
                                    <div class="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 rounded-lg">
                                        In Progress
                                    </div>
                                </div>
                            </div>
                    ...
                    <div v-if="!isExplorer" class="bento-card p-10 bg-slate-900 dark:bg-indigo-950 text-white relative overflow-hidden group cursor-pointer" @click="demandAccess('ai_coach', route('coach.index'))">
                        ...
                        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10">
                            <div class="max-w-xl">
                                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 text-indigo-300 text-[9px] font-black mb-6 uppercase tracking-[0.2em] backdrop-blur-md border border-white/5 animate-pulse">
                                    <Sparkles :size="12" />
                                ...
                                <p class="text-indigo-200/60 text-xs font-bold uppercase tracking-widest">Tap to open Neural Hub <ArrowRight :size="10" class="inline ml-1" /></p>
                            </div>
                    ...
                    <div v-else class="bento-card p-10 flex flex-col items-center text-center group">
                        ...
                        <Link :href="route('billing')" class="px-8 py-3.5 bg-slate-900 dark:bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-100 dark:shadow-none transition-all active:scale-95">
                            {{ $t('dash_upgrade_tier') }}
                        </Link>
                    </div>
                </div>
                ...
                <div class="col-span-12 lg:col-span-4 space-y-6 md:space-y-8">
                    ...
                    <div class="bento-card p-8 group">
                        <div class="flex items-center justify-between mb-8">
                            ...
                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ synergy?.habits.completed }}/{{ synergy?.habits.total }} Done</span>
                        </div>
                    ...
                    <div @click="demandAccess('finance_trends', route('finance.index'))" 
                        ...
                        <div v-if="!isExplorer" class="space-y-4">
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{{ $t('dash_total_expenses') }}</p>
                        ...
                        <div v-else class="space-y-4">
                            ...
                            <p class="text-[9px] font-black text-indigo-500 uppercase tracking-widest">{{ $t('dash_unlock_arch') }}</p>
                        </div>
                    </div>
            ...
            <div v-if="isExplorer" class="mt-12 group">
                <Link :href="route('billing')" class="block bg-white dark:bg-slate-900 rounded-[3rem] p-10 md:p-12 border border-slate-100 dark:border-white/5 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden active:scale-[0.99] ultra-shadow">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10">
                        ...
                        <div class="px-10 py-5 bg-slate-900 dark:bg-indigo-600 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.2em] shadow-xl flex items-center gap-3 transition-transform group-hover:scale-105">
