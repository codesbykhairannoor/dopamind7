...
<template>
    <section>
        <header>
            <h2 class="text-lg font-black text-slate-800 dark:text-white tracking-tight transition-colors duration-500">{{ $t('profile_info_title', 'Informasi Pribadi') }}</h2>
        ...
        <div class="mt-8 space-y-6">
            ...
            <div class="flex items-center gap-6 p-5 bg-slate-50 dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 transition-colors duration-500">
                ...
                <div class="flex-1">
                    ...
                    <button type="button" @click="selectNewPhoto" :disabled="form.processing" class="px-5 py-2.5 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-xl text-[11px] font-black tracking-wide text-indigo-600 dark:text-indigo-400 shadow-sm dark:shadow-none hover:border-indigo-300 dark:hover:border-indigo-500/50 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-all active:scale-95 disabled:opacity-50 transition-colors duration-500">
            ...
            <form @submit.prevent="submitTextForm">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <InputLabel for="name" :value="$t('full_name', 'Nama Lengkap')" class="!text-[10px] !font-black !tracking-wide !text-slate-400 dark:!text-slate-500 mb-2 transition-colors duration-500" />
                    ...
                    <div>
                        <InputLabel for="email" :value="$t('email_address', 'Alamat Email')" class="!text-[10px] !font-black !tracking-wide !text-slate-400 dark:!text-slate-500 mb-2 transition-colors duration-500" />
                ...
                <div class="flex items-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors duration-500">
                    <PrimaryButton :disabled="form.processing" class="!rounded-xl !py-3.5 !px-8 !bg-indigo-600 hover:!bg-indigo-700 !shadow-lg !shadow-indigo-200 dark:!shadow-none !text-[11px] !font-black !tracking-wide transition-all active:scale-95 transition-colors duration-500">
            ...
            <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
                <div v-if="showCropper" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white dark:bg-slate-900 p-6 rounded-[2rem] w-full max-w-sm shadow-2xl dark:shadow-none border border-slate-100 dark:border-slate-800 flex flex-col items-center transition-colors duration-500">
                        ...
                        <p class="mt-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 tracking-wide transition-colors duration-500">{{ $t('pan_instruction', 'Gunakan Mouse/Sentuhan Untuk Menggeser') }}</p>
                        ...
                        <div class="flex gap-3 mt-8 w-full">
                            <button @click="cancelCrop" type="button" class="flex-1 py-3.5 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-2xl font-black text-xs tracking-wide border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 active:scale-95 transition-all transition-colors duration-500">
                                {{ $t('cancel', 'Batal') }}
                            </button>
                            <button @click="applyCropAndUpload" type="button" class="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-xs tracking-wide shadow-lg shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 active:scale-95 transition-all">
