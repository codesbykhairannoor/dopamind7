        ...
        <div class="mt-8 space-y-6">
            ...
            <div class="flex items-center gap-6 p-5 bg-slate-50 rounded-3xl border border-slate-100">
                <div class="relative group cursor-pointer shrink-0" @click="selectNewPhoto">
                    <img :src="photoPreview || user.avatar_url" alt="Profile" class="w-24 h-24 rounded-full object-cover shadow-md border-4 border-white transition-all duration-300 group-hover:brightness-75 group-hover:scale-105">
                ...
                <div class="flex-1">
                    ...
                    <button type="button" @click="selectNewPhoto" :disabled="form.processing" class="px-5 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-[11px] font-black uppercase tracking-widest text-indigo-600 shadow-sm hover:border-indigo-300 hover:bg-indigo-50 transition-all active:scale-95 disabled:opacity-50">
            ...
            <form @submit.prevent="submitTextForm">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <TextInput id="name" type="text" class="mt-1 block w-full !rounded-xl !border-slate-200 focus:!border-indigo-500 focus:!ring-0 font-bold text-slate-700 h-12 px-4" v-model="form.name" required autofocus autocomplete="name" />
                    ...
                    <div>
                        ...
                        <TextInput id="email" type="email" class="mt-1 block w-full !rounded-xl !border-slate-200 focus:!border-indigo-500 focus:!ring-0 font-bold text-slate-700 h-12 px-4" v-model="form.email" required autocomplete="username" />
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
                <div v-if="showCropper" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white p-6 rounded-[2rem] w-full max-w-sm shadow-2xl border border-slate-100 flex flex-col items-center">
                        <h3 class="font-black text-slate-800 mb-6 uppercase tracking-widest text-sm">{{ $t('adjust_photo_title', 'Sesuaikan Foto') }}</h3>
                        
                        <div 
                            class="relative w-48 h-48 bg-slate-100 rounded-full overflow-hidden cursor-move border-4 border-dashed border-indigo-200 shadow-inner"
                        ...
                        <p class="mt-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('pan_instruction', 'Gunakan Mouse/Sentuhan Untuk Menggeser') }}</p>

                        <div class="w-full mt-6 flex items-center gap-3">
                            <span class="text-sm text-slate-400">🔍</span>
                            <input type="range" v-model="zoom" min="0.5" max="2.5" step="0.05" class="w-full accent-indigo-600">
                        </div>

                        <div class="flex gap-3 mt-8 w-full">
                            <button @click="cancelCrop" type="button" class="flex-1 py-3.5 bg-slate-50 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest border border-slate-200 hover:bg-slate-100 active:scale-95 transition-all">
                                {{ $t('cancel', 'Batal') }}
                            </button>
                            <button @click="applyCropAndUpload" type="button" class="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">
