    ...
    <Modal :show="show" @close="handleClose">
        <div class="bg-white dark:bg-slate-900 rounded-2xl relative overflow-hidden transition-all duration-300 border border-slate-100 dark:border-slate-800 shadow-2xl dark:shadow-none">
            ...
            <div v-if="isConfirmingDeletion" class="p-8 text-center animate-in fade-in zoom-in duration-200 bg-white dark:bg-slate-900 transition-colors duration-500">
                ...
                <h3 class="text-xl font-black text-slate-800 dark:text-slate-100 mb-2 transition-colors duration-500">
    {{ $t('delete_activity_title') }}
</h3>
<p class="text-slate-500 dark:text-slate-400 text-sm mb-8 px-4 font-medium leading-relaxed transition-colors duration-500">
    {{ $t('delete_activity_desc', { title: form.title || $t('default_activity_name') }) }}
</p>
                <div class="flex gap-3 justify-center">
                    <button @click="cancelDelete" class="px-6 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition active:scale-95">
                        {{ $t('btn_cancel') }}
                    </button>
                    <button @click="confirmDelete" class="px-6 py-2.5 rounded-xl bg-rose-500 text-white font-bold shadow-lg shadow-rose-200 dark:shadow-none hover:bg-rose-600 transition transform active:scale-95">
                        {{ $t('btn_delete') }}
                    </button>
                </div>
            </div>
            ...
            <div v-else>
                ...
                <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center mt-1 transition-colors duration-500" :class="{'mt-10': conflictError}">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-xl text-white shadow-lg shadow-indigo-100 dark:shadow-none">
                            ✨
                        </div>
                        <div>
