...
onMounted(() => {
    ...
    if (savedPlatinum !== null)
        platinumExpanded.value = savedPlatinum !== "false";
...
<template>
    <div
        class="flex flex-col h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden selection:bg-indigo-500 selection:text-white relative transition-colors duration-500"
    >
        ...
        <div class="flex flex-1 overflow-hidden relative">
            ...
            >
                <!-- ---- SCROLLABLE NAV AREA ---- -->
                <nav
                    class="flex-1 overflow-y-auto py-3 custom-scrollbar space-y-0.5"
                    :class="isSidebarCollapsed ? 'px-2' : 'px-2.5'"
                >
                    <AppSidebarNav
                        variant="sidebar"
                ...
                <div class="md:hidden">
                    ...
                    <Transition name="mobile-drawer-panel">
                        ...
                        >
                            ...
                            <nav
                                class="custom-scrollbar flex-1 overflow-y-auto py-2"
                            >
                                <AppSidebarNav
                                    variant="drawer"
                                    :collapsed="false"
            ...
            <!-- MAIN CONTENT AREA -->
            <main
                class="flex-1 overflow-y-auto relative w-full bg-slate-50 dark:bg-slate-950 pb-32 md:pb-0 transition-colors duration-500 custom-scrollbar"
            >
        ...
        >
            ...
            >
                ...
                ></div>
                <div
                    class="relative bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden"
                >
                    <div class="p-8 text-center">
                        <div
                            class="w-20 h-20 bg-rose-50 dark:bg-rose-500/10 text-rose-500 rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-inner"
                        >
                            <OneForMindIcon
                                name="logout"
                                size="40"
                        ...
                        <h3
                            class="text-2xl font-black text-slate-800 dark:text-white tracking-tight mb-2"
                        >
                            {{ $t("modal_logout_title", "Keluar Akun?") }}
                        </h3>
                        <p
                            class="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed"
                        >
                            {{
                                $t(
                    ...
                    <div
                        class="p-6 bg-slate-50/80 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-3"
                    >
                        <button
                            @click="router.post(route('logout'))"
                            class="w-full bg-rose-500 text-white font-black py-4 rounded-2xl shadow-lg shadow-rose-200 dark:shadow-none hover:bg-rose-600 active:scale-[0.98] transition-all"
                        >
                            {{ $t("modal_logout_confirm", "Ya, Keluar") }}
                        </button>
                        <button
                            @click="showLogoutModal = false"
                            class="w-full bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold py-4 rounded-2xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 active:scale-[0.98] transition-all"
                        >
                            {{ $t("modal_logout_cancel", "Batal") }}
                        </button>
                    </div>
                </div>
            </div>
        ...
        >
            <div class="relative">
                <div
                    class="absolute inset-0 bg-indigo-500 blur-2xl opacity-20 group-hover:opacity-40 transition-opacity rounded-full"
                ></div>
                <div
                ...
                >
                    <div
                        class="absolute inset-0 bg-gradient-to-tr from-indigo-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"
                    ></div>
                    <OneForMindIcon
                        name="sparkles"
                        size="24"
