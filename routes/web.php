...
Route::post("/settings/cookie-consent", function (Request $request) {
    if (auth()->check()) {
        auth()
            ->user()
            ->update(["cookie_consent" => $request->consent]);
    }
...
Route::get("/company/contact", [CompanyController::class, "contact"])->name(
    "company.contact",
);
Route::post("/company/contact", [
    CompanyController::class,
    "sendContact",
])->name("contact.send");

Route::get("/company/privacy", [CompanyController::class, "privacy"])->name(
    "company.privacy",
);
Route::get("/company/terms", [CompanyController::class, "terms"])->name(
    "company.terms",
);
Route::get("/company/refund", [CompanyController::class, "refund"])->name(
    "company.refund",
);
Route::get("/company/security", [CompanyController::class, "security"])->name(
    "company.security",
);
...
Route::middleware(["auth", "verified", "onboarding", "throttle:global"])->group(function () {
    ...
    Route::middleware(["module:planner"])
        ...
        ->group(function () {
            ...
            Route::patch("/{plannerTask}", [
                PlannerController::class,
                "update",
            ])->name("update");
            Route::delete("/{plannerTask}", [
                PlannerController::class,
                "destroy",
            ])->name("destroy");
            Route::patch("/{plannerTask}/toggle", [
                PlannerController::class,
                "toggle",
            ])->name("toggle");
            Route::post("/log", [PlannerController::class, "updateLog"])->name(
                "updateLog",
            );
            Route::post("/reset", [
                PlannerController::class,
    ...
    Route::middleware(["module:habit"])
        ...
        ->group(function () {
            ...
            Route::post("/batch", [HabitController::class, "batchStore"])->name(
                "batchStore",
            );
            Route::post("/copy", [
            ...
            Route::post("/mood", [HabitController::class, "updateMood"])->name(
                "mood",
            );

            // 🔥 PERBAIKAN DI SINI: hapus '/habits' dan 'habits.' karena sudah ada di grup atas
            Route::post("/reorder", [HabitController::class, "reorder"])->name(
                "reorder",
            );
            Route::post("/batch-log", [
                HabitController::class,
                "batchLog",
            ])->name("batch-log");

            // Ini harus di bawah supaya tidak bentrok (Laravel membaca /reorder dulu baru /{habit})
            Route::patch("/{habit}", [HabitController::class, "update"])->name(
                "update",
            );
            Route::delete("/{habit}", [
                HabitController::class,
    ...
    Route::middleware(["module:finance"])
        ...
        ->group(function () {
            ...
            Route::patch("/transaction/{financeTransaction}", [
                FinanceController::class,
                "updateTransaction",
            ])->name("transaction.update");
            Route::delete("/transaction/{financeTransaction}", [
                FinanceController::class,
                "destroyTransaction",
            ])->name("transaction.destroy");

            // 🔥 FIX BATCH ENTRY: Hilangkan kata 'finance' di URL dan Name
            Route::post("/transactions-batch", [
                FinanceController::class,
                "batchStoreTransaction",
            ])->name("transaction.batchStore");

            Route::get("/category/check/{category}", [
                FinanceController::class,
                "checkCategoryUsage",
            ])->name("category.check");
            Route::post("/category/rename", [
                FinanceController::class,
                "renameCategory",
            ...
            Route::middleware("subscription:pro")->group(function () {
                ...
                Route::put("/budget/{financeBudget}", [
                    FinanceController::class,
                    "updateBudget",
                ])->name("budget.update");
                Route::delete("/budget/{financeBudget}", [
                    FinanceController::class,
                    "destroyBudget",
                ])->name("budget.destroy");
            });
            ...
            Route::prefix("savings")
                ...
                ->group(function () {
                    Route::post("/", [
                        \App\Http\Controllers\FinanceSavingController::class,
                        "store",
                    ])->name("store");
                    Route::patch("/{financeSaving}", [
                        \App\Http\Controllers\FinanceSavingController::class,
                        "update",
                    ])->name("update");
                    Route::delete("/{financeSaving}", [
                        \App\Http\Controllers\FinanceSavingController::class,
                        "destroy",
                    ])->name("destroy");
                    Route::post("/{financeSaving}/deposit", [
                        \App\Http\Controllers\FinanceSavingController::class,
                        "deposit",
                    ])->name("deposit");
                    Route::post("/{financeSaving}/withdraw", [
                        \App\Http\Controllers\FinanceSavingController::class,
                        "withdraw",
                    ])->name("withdraw");
                });
    ...
    // --- MODULE: JOURNAL ---
    Route::middleware(["module:journal"])
        ->prefix("journal")
        ->name("journal.")
        ->group(function () {
            ...
            // Sistem CRUD
            Route::post("/", [JournalController::class, "store"])->name(
                "store",
            );
            Route::patch("/{id}", [JournalController::class, "update"])->name(
                "update",
            );
            Route::delete("/{id}", [JournalController::class, "destroy"])->name(
                "destroy",
            );
    ...
    // --- MODULE: CALENDAR ---
    Route::middleware(["module:calendar"])
        ->prefix("calendar")
        ->name("calendar.")
        ->group(function () {
    ...
    // Goal Tracker Routes
    Route::middleware(["module:goal", "subscription:pro"])
        ->prefix("goals")
        ->name("goals.")
        ->group(function () {
            Route::get("/", [
            ...
            // Milestones
            Route::post("/{goal}/milestones", [
                \App\Http\Controllers\GoalController::class,
                "storeMilestone",
            ])->name("milestones.store");
            Route::patch("/{goal}/milestones/{milestone}", [
                \App\Http\Controllers\GoalController::class,
                "updateMilestone",
            ])->name("milestones.update");
            Route::post("/{goal}/milestones/{milestone}/toggle", [
                \App\Http\Controllers\GoalController::class,
                "toggleMilestone",
            ])->name("milestones.toggle");
            Route::delete("/{goal}/milestones/{milestone}", [
                \App\Http\Controllers\GoalController::class,
                "destroyMilestone",
    ...
    Route::prefix("coach")
        ...
        ->group(function () {
            ...
            Route::post("/habit-stack", [
                \App\Http\Controllers\AiHabitController::class,
                "suggestStack",
            ])->name("habit.stack");
            Route::post("/habit-mood", [
                \App\Http\Controllers\AiHabitController::class,
                "adaptToMood",
            ])->name("habit.mood");
            Route::post("/habit-audit", [
                \App\Http\Controllers\AiHabitController::class,
                "auditFriction",
            ])->name("habit.audit");
            Route::post("/habit-stagnation", [
                \App\Http\Controllers\AiHabitController::class,
                "auditStagnation",
            ])->name("habit.stagnation");

            // Debugging
            Route::get("/debug-gemini", [
                \App\Http\Controllers\AiDebugController::class,
                "testGemini",
            ]);
        });
    ...
    Route::prefix("study")
        ...
        ->group(function () {
            ...
            Route::get("/portfolio/{id}/logs", [StudyController::class, "viewLogs"])->name("portfolio.logs");
            Route::get("/portfolio/stream-logs", [StudyController::class, "streamLogs"])->name("portfolio.stream_logs");
        ...
        ->group(function () {
            ...
            Route::prefix("users")
                ...
                ->group(function () {
                    ...
                    Route::post("/{user}/toggle-premium", [
                        AdminUserController::class,
                        "togglePremium",
                    ])->name("toggle-premium");
                    Route::post("/{user}/toggle-admin", [
                        AdminUserController::class,
                        "toggleAdmin",
                    ])->name("toggle-admin");
                });
